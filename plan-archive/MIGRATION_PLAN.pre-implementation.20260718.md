...
- [ ] Taxation (calls `tax_salary_process_prc`) — 🟡 PARTIAL: per-employee tax-declaration entry exists (`employees/[id]/tax-declarations`); the actual tax calculation run is not started
- [ ] Arrears processing — not started
- [ ] Year-end processing — not started
- [ ] Salary slip generation (PDF via jspdf-autotable) — not started
- [ ] Variable pay management — not started

**Milestone: Can process payroll for a month and generate payslips. — ❌ NOT REACHED. See `reports/migration-plan.md` Phase 3, which sequences this after Leave.**

### Phase 6: Reports (8–10 days)

- [ ] Salary reports — not started
- [ ] Salary slip bulk download (multiple employees) — not started
- [ ] Attendance reports — not started
- [ ] ESI/EPF statutory reports — not started
- [ ] Statutory registers — not started
- [ ] LOP report — not started
- [ ] Employee reports — not started
- [ ] Advance/loan/increment reports — not started
- [ ] All reports: Excel export (SheetJS) + PDF export (jspdf-autotable) — not started, though both libraries are already dependencies and already used elsewhere (document generation, resignation slips)

**Milestone: All core reports functional with export. — ❌ NOT REACHED. `reports/migration-plan.md` recommends building these incrementally alongside each owning module rather than as one big-bang phase.**
...
- [ ] Performance management — not started (Promotions and Resignations, tracked elsewhere in the legacy controller list, ARE built — see the note above the table)
- [ ] Site & field operations — not started
- [ ] Procurement / inventory — not started
- [ ] Mobile tracking — not started

### Phase 9: Polish & Cutover (7–10 days)

- [ ] Migrate Slim PHP API v1 endpoints to Next.js API routes — not started
- [ ] End-to-end testing with real data (parallel run with legacy) — not started
- [ ] Performance testing (connection pool under load) — not started
- [ ] UI responsiveness and mobile audit — not started
- [ ] Error boundary and loading state coverage — not confirmed this session
- [ ] Production environment setup — not confirmed this session

**Total Estimate: 73–97 working days (≈ 15–20 weeks) — original estimate; not re-baselined against actual velocity.**
