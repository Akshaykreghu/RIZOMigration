...
class EmployeeExpenseReportsController extends AppController {
     ...
     * @var array
     */
    public $uses = array('EditPunches', 'Attendance', 'CentralControl', 'UserCredentials', 'EmployeeDetails', 'EmployeeProfessionalDetails', 'DeviceAttendance', 'Departments', 'Grades', 'Verticals', 'Units', 'ReportCriterias', 'AttendanceRegister', 'AttendanceRegisterReport', 'DbConfig', 'MobileUserauditor', 'EmployeeExpenses'); //santhu
...
//debug($mode);
    ...
    private function generateemployeeexpence($mode = '') {
        ...
        $arr_employee_expense = $this->EmployeeExpenses->query("select ex.*,ei.*,EmployeeDetails.branch_code,Units.branch_name
        ...
        if ($needBranchWiseReport) {
            ...
            foreach ($arr_employee_expense as $employee_expenses) {
                ...
                if ($branch_code != '') {
                    ...
                    $request['branch'] = isset($employee_expenses['eu']['branch']) ? $employee_expenses['ei']['branch'] : '';
        ...
        switch ($mode) {
            case 'pdf' :
                ...
                $html2pdf->Output('Emp_Expenses_report.pdf', 'D');
                //$this->render('reportleavepolicy');
                break;
            case 'excel' :
                $str_company_code = $this->Session->read('company_code');
                $file_name = isset($str_company_code) ? $str_company_code . "Emp_Expenses_report.xlsx" : "ShiftPolicy" . strtotime() . ".xlsx";
                ...
                $worksheet->setCellValueByColumnAndRow(0, 1, "Employee Expenses Report");
...
//                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 5), $rowcount)->getFont()->setBold(true);
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 5), $rowcount, 'Expenses Amount');
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 5), $rowcount)->getFont()->setBold(true);
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 6), $rowcount, 'Incurred  Date');
                        ...
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 8), $rowcount, 'Remark');
...
//                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 5), ($rowcount), $des);
                                ...
                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 8), $rowcount, $rem);
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 5), $rowcount, 'Branch');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 5), $rowcount)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 6), $rowcount, 'Expenses Amount');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 6), $rowcount)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 7), $rowcount, 'Incurred Date');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 7), $rowcount)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 8), $rowcount, 'Created Date');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 8), $rowcount)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 9), $rowcount, 'Remark');
                    ...
                    if (count($arr_data) >= 0) {
                        ...
                        foreach ($arr_data as $val) {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 9), $rowcount, $rem);
