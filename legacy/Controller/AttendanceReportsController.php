...
class attendanceReportsController extends AppController {
    ...
    public function hrreports() {
        ...
        $user = strtoupper($this->Session->read('company_code'));
          if ($user == 'GLET' || $user == 'GAAR' || $user == 'ABSG') {
           ...
           // 'nonattendance' => 'Non-Attendance',
            ...
            if ($plan == 'basic') {
                $arr_reporttypes = array(
                    'VerifiedAttendance' => 'Attendance Register',
                    'DetailedAttendance' => 'Detailed Attendance Reports',
                    'Dashboard' => 'Employee Check-in/out logs',
                );
            } else {
        ...
        if (isset($model) && $model != '') {
            ...
            switch ($model) {
                ...
                case 'EmployeeDetails':
                    ...
                    $arr_order = array("CONCAT(EmployeeDetails.first_name, ifnull(EmployeeDetails.last_name,''))" => "ASC"); //Edited by Akshay on 23-2-2024
    ...
    public function downloadHistory($type, $mode) {
        ...
        switch ($type) {
            ...
            case 'nonpunched':
                $dataForHistory['report_type'] = "Employee Non Punched Report";
                break;
            case 'nonattendance':
                $dataForHistory['report_type'] = "Employee Non Attendance Report";
            default:
                // End
...
//attendance register report
       private function generatesummaryreport($mode)
    {
        ...
        if (isset($arr_leavepolicygroupids) && !empty($arr_leavepolicygroupids)) {
            foreach ($arr_leavepolicygroupids as $leavepolicygroupid) {
                // debug($leavepolicygroupid);
                $fields = 'AttendanceRegister.*,EmployeeDetails.status,Termination.last_approved_working_date,Branch.branch_name,config.hirc_leval,Info.*';
                ...
                $joins = array(
                    array(
                        ...
                        'foreignKey' => false,
                        'conditions' => array(
                            'AttendanceRegister.branch_code = Branch.branch_code', 'Branch.status = 1'
       ...
       if ($att_startdate != 1) {
        ...
        if (isset($arr_leavepolicygroupids) && !empty($arr_leavepolicygroupids)) {
            foreach ($arr_leavepolicygroupids as $leavepolicygroupid) {
                ...
                $fields = 'AttendanceRegister.*,Termination.last_approved_working_date,Branch.branch_name,Info.*';
...
//Over Time
    ...
    public function generateOvertimereport($type = '', $mode = '')
    {
        ...
        $this->EmployeeDetails->useDbConfig = $this->Session->read('ds');
        $fd = $arr_form_data['reportfrom'] . ' ' . '00:00:00';
        $user_id = $this->Session->read('login_user_id');  $date_time = date('d-m-Y H:i');
        // $Td=$arr_form_data['reportto'].' '.'00:00:00';
        if ((isset($arr_form_data['reportfrom']) && $arr_form_data['reportfrom'] != '')) {
            $report_month = $arr_form_data['reportfrom'];
...
//            );
	       ...
	       //$arr_resp['data'][$i][]/* ['Location'] */ = isset($att["DeviceAttendance"]['C3']) ? $att["DeviceAttendance"]['C3'] : '';//This location added by ARUL P DAS on 22_4_2020
            ...
            switch ($mode) {
                ...
                case 'excel' :
                    $str_company_code = $this->Session->read('company_code');
                    $file_name = !empty($str_company_code) ? $str_company_code . "_CheckinLogs.xlsx" : "CheckinLogs_" . time() . ".xlsx";
                    ...
                    $sheet->setCellValue('A2', 'Report Period: ' . $report_month);
                    $sheet->mergeCells('A2:K2');
                    $sheet->getStyle('A2')->getFont()->setBold(true)->setSize(12);
                    $sheet->getStyle('A2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

                    $headers = array('Sl No', 'Employee ID', 'Employee Name', 'Branch', 'Department', 'Designation', 'Date', 'Time', 'Check-In/Out', 'Location', 'Punch Type');
                    ...
                    $row = 4;
                    foreach ($arr_resp['data'] as $idx => $val) {
                        $sheet->setCellValueByColumnAndRow(0, $row, $idx + 1);
                        $sheet->setCellValueByColumnAndRow(1, $row, $val[0]);
                        $sheet->setCellValueByColumnAndRow(2, $row, $val[1] . ($val[10] == 2 ? ' (Resigned)' : ''));
                    ...
                    $objPHPExcel->getActiveSheet()->setTitle('Checkin Logs');
                    $objWriter = new PHPExcel_Writer_Excel2007($objPHPExcel);
                    $objWriter->save(dirname(__FILE__) . "/" . $file_name);
   ...
   private function generateregularisationreport($type, $mode){
        ...
        $from = date('Y-m', strtotime($arr_form_data['reportfrom']));
        $company_code = $this->Session->read('company_code'); //company_code
        $fromdate = date('Y-m-d', strtotime($arr_form_data['reportfrom']));
        $todate = date("Y-m-d", strtotime($arr_form_data['reportto']));
                    ...
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 18), $rowcount)->getFont()->setBold(true);
                        ...
                        foreach($arr_lateoutdata_for_template as $value){
                         ...
                         if(count($arr_data)>=0){
                            ...
                            $att_date=$val['employee_regularaization']['att_date'];
                            $time=$val['employee_regularaization']['LOGTIME'];
                            $direction=$val['employee_regularaization']['C1'];
                            $c_date=$val['employee_regularaization']['created_date'];
                            ...
                            $approved_rejected_date=$val['employee_regularaization']['updated_date'];
                            }else{
    ...
    /* header footer */
     ...
     public function generatenonattendancereport($type, $mode)
     {
         ...
         $startDate = (new DateTime($reporttfrom))->modify('-1 month');
         $endDate = (new DateTime($reportto))->modify('+1 month');
