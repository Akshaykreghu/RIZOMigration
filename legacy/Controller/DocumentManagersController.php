 ...
 */
class DocumentManagersController extends AppController
{
     ...
     * @var array
     */
    public $uses = array('DocTemplate', 'TemplatesDetails', 'Templates', 'Documents', 'EmpDetails', 'Units', 'EmployeeDetails', 'EmployeeExpenses', 'DocumentAllocation', 'DocumentUpload', 'Country');
    ...
    public function form($template_pkey = '')
    {
        ...
        if ($template_pkey != '') {
            ...
            $result = $this->DocTemplate->query('select template_name ,template_content,placeholders,availability, editable, policy, header_image, footer_image from doc_template where  template_pkey=' . $template_pkey . ' and status = 1 ');
            ...
            if (isset($result[0]) && isset($result[0]['doc_template'])) {
                ...
                $this->set('header_image', (isset($result[0]['doc_template']['header_image']) ? $result[0]['doc_template']['header_image'] : ""));
                $this->set('footer_image', (isset($result[0]['doc_template']['footer_image']) ? $result[0]['doc_template']['footer_image'] : ""));
            }
        }
    ...
    public function saveTemplate()
    {
        ...
        // edited by bindu 27-05-2026
        $upload_dir = "./img/";
        if (!file_exists($upload_dir)) {
            mkdir($upload_dir, 0755, TRUE);
        }
        $finfo = new finfo(FILEINFO_MIME_TYPE);
        $allowed_mime = array('jpg' => 'image/jpeg', 'png' => 'image/png', 'gif' => 'image/gif', 'jpeg' => 'image/jpeg');
        $file_field_map = array('header_image_file' => 'header_image', 'footer_image_file' => 'footer_image');
        foreach ($file_field_map as $file_field => $db_field) {
            if (isset($_FILES[$file_field]) && $_FILES[$file_field]['error'] === 0) {
                $ext = array_search($finfo->file($_FILES[$file_field]['tmp_name']), $allowed_mime, true);
                if ($ext !== false) {
                    $filename = sprintf('%s.%s', sha1_file($_FILES[$file_field]['tmp_name']), $ext);
        ...
        if ($action == 'add') {
            ...
            // header_image/footer_image excluded here — saved via updateAll below to bypass schema cache
            ...
            $savedPkey = $data['template_pkey'];
        }
        // Save header/footer image paths via direct updateAll (bypasses CakePHP schema cache,
        // and only fires when a file was actually uploaded — preserves existing values otherwise)
        $imgUpdate = array();
        // Only update image columns when a non-empty path arrived (i.e. a new file was uploaded).
        // Using !empty() instead of isset() prevents wiping existing DB values when the user
        // submits the form without choosing a new image (the hidden field would be "" in that case).
        // edited by bindu 27-05-2026
        if (!empty($data['header_image'])) {
            $imgUpdate['DocTemplate.header_image'] = "'" . $data['header_image'] . "'";
        }
        if (!empty($data['footer_image'])) {
            $imgUpdate['DocTemplate.footer_image'] = "'" . $data['footer_image'] . "'";
        }
        if (!empty($imgUpdate) && !empty($savedPkey)) {
            $this->DocTemplate->updateAll($imgUpdate, array('DocTemplate.template_pkey' => $savedPkey));
        }
    ...
    public function getDocuments($asArray = false)
    {
        ...
        $result = $this->Documents->query("
          SELECT 
            CASE 
                WHEN em.emp_pkey IS NOT NULL 
                    THEN CONCAT(COALESCE(em.first_name, ''), ' ', COALESCE(em.middile_name, ''), ' ', COALESCE(em.last_name, ''))
                WHEN ej.emp_join_pkey IS NOT NULL
                    THEN CONCAT(COALESCE(ej.first_name, ''), ' ', COALESCE(ej.last_name, ''))
                ELSE ''
            END AS emp_name,
            tem.template_name,
            doc.document_name,
            ...
            doc.doc_id
        FROM documents AS doc
        LEFT JOIN emp_details AS em 
               ON em.emp_pkey = doc.emp_fkey AND em.status IN (1,2)
        LEFT JOIN emp_join AS ej 
               ON ej.emp_join_pkey = doc.emp_join_fkey
        LEFT JOIN doc_template AS tem 
               ON tem.template_pkey = doc.template_fkey AND tem.status = 1
        WHERE doc.status = 1
        ...
        foreach ($result as $key => $val) {
            ...
            $url = Router::url('/', true) . 'DocumentManagers/docPreview/' . $val['doc']['document_pkey'] . '/1';
            ...
            $documents[] = array_merge(
                ...
                $val[0],
                $val['doc'],
                array('download_link' => $downloadLink)
            );
    ...
    public function docPreview($document_id = '', $preview = 0)
    {
        ...
        if (isset($document_id) && !empty($document_id)) {
            $data = $this->Documents->query("select d.document_name, d.document, d.creation_date, d.created_by, d.doc_id, dt.header_image, dt.footer_image from documents d left join doc_template dt on dt.template_pkey = d.template_fkey and dt.status = 1 where d.document_pkey = " . $document_id . "");
            $date = isset($data[0]['d']['creation_date']) ? $data[0]['d']['creation_date'] : '';
            $created = isset($data[0]['d']['created_by']) ? $data[0]['d']['created_by'] : '';
            $html = (isset($data[0]) && isset($data[0]['d']['document'])) ? $data[0]['d']['document'] : '';
            $doc = (isset($data[0]) && isset($data[0]['d']['doc_id'])) ? $data[0]['d']['doc_id'] : '';
            $header_image = (isset($data[0]['dt']['header_image'])) ? $data[0]['dt']['header_image'] : '';
            $footer_image = (isset($data[0]['dt']['footer_image'])) ? $data[0]['dt']['footer_image'] : '';
            $header_path = $header_image;
            if (!empty($this->webroot) && $this->webroot !== '/' && strpos($header_path, $this->webroot) === 0) {
                $header_path = substr($header_path, strlen($this->webroot));
            }
            $header_image_fs = !empty($header_image) ? 'file://' . WWW_ROOT . ltrim($header_path, '/') : '';

            $footer_path = $footer_image;
            if (!empty($this->webroot) && $this->webroot !== '/' && strpos($footer_path, $this->webroot) === 0) {
                $footer_path = substr($footer_path, strlen($this->webroot));
            }
            $footer_image_fs = !empty($footer_image) ? 'file://' . WWW_ROOT . ltrim($footer_path, '/') : '';
            $header_h = !empty($header_image_fs) ? 100 : 15;
            $footer_h = !empty($footer_image_fs) ? 90 : 15;
            $wrapped = '';
            $wrapped .= '<style>
                @page {
                ...
                #doc-page-header img { width: 100%; height: 100%; display: block; }
                #doc-page-footer {
                    position: fixed;
                    bottom: -' . $footer_h . 'px;
                    left: -25px;
                ...
                #doc-page-footer img { width: 100%; height: 100%; display: block; }
                #footer { position: fixed; left: 0; bottom: 5px; right: 0; height: 20px; text-align: center; padding: 10px; font-size: 12px; }
            </style>';
            if (!empty($header_image_fs)) {
                $wrapped .= '<div id="doc-page-header"><img src="' . htmlspecialchars($header_image_fs, ENT_QUOTES, 'UTF-8') . '" alt="Header"></div>';
            }
            if (!empty($footer_image_fs)) {
                $wrapped .= '<div id="doc-page-footer"><img src="' . htmlspecialchars($footer_image_fs, ENT_QUOTES, 'UTF-8') . '" alt="Footer"></div>';
            }
            //edited by sinsiya on 20-09-2024
            if ($companycode != 'ayrg' && $companycode != 'demo') {
                $wrapped .= '<div id="footer"> Generated by ' . $created . ' at ' . $date . ' with Document ID ' . $doc . '</div>';
            }
            $wrapped .= $html;
            $html = $wrapped;
            ...
            $emp_join_fkey = (isset($_POST['emp_join_id']) && !empty($_POST['emp_join_id'])) ? $_POST['emp_join_id'] : 0;
            ...
            $preview_footer_image = '';
            $temp_id = (isset($_POST['temp_id']) && !empty($_POST['temp_id'])) ? (int) $_POST['temp_id'] : 0;
            if ($temp_id > 0) {
                $tplData = $this->Documents->query("select header_image, footer_image from doc_template where template_pkey = " . $temp_id . " and status = 1");
                $preview_header_image = isset($tplData[0]['doc_template']['header_image']) ? $tplData[0]['doc_template']['header_image'] : '';
                $preview_footer_image = isset($tplData[0]['doc_template']['footer_image']) ? $tplData[0]['doc_template']['footer_image'] : '';
                $preview_header_path = $preview_header_image;
                if (!empty($this->webroot) && $this->webroot !== '/' && strpos($preview_header_path, $this->webroot) === 0) {
                    $preview_header_path = substr($preview_header_path, strlen($this->webroot));
                }
                $preview_header_image = !empty($preview_header_image) ? 'file://' . WWW_ROOT . ltrim($preview_header_path, '/') : '';

                $preview_footer_path = $preview_footer_image;
                if (!empty($this->webroot) && $this->webroot !== '/' && strpos($preview_footer_path, $this->webroot) === 0) {
                    $preview_footer_path = substr($preview_footer_path, strlen($this->webroot));
                }
                $preview_footer_image = !empty($preview_footer_image) ? 'file://' . WWW_ROOT . ltrim($preview_footer_path, '/') : '';
            }
            ...
            //End
            // edited by bindu 25-10-25
            $employee_join_name = $employee_join_avatar = $employee_join_mobile_no = $employee_join_country = $employee_join_date_of_birth = $employee_join_email = $employee_join_address = $employee_join_pincode = $employee_join_district = $employee_join_guardian = $employee_join_gender = $employee_join_state = '';

            $employee_join_bank_name = $employee_join_branch_name = $employee_join_ifsc_code = $employee_join_account_no = '';

            $employee_join_pf = $employee_join_uan = $employee_join_previous_member = $employee_join_esi_dispensary = $employee_join_esi = $employee_join_eps = $employee_join_pan_no = $employee_join_wps_code = $employee_join_maritual_status = $employee_join_relation_guardian = $employee_join_lwf_code = $employee_join_blood_group = $employee_join_issue_date = '';
            // end
            //Edited by Akshay on 15-7-2024  lwf_code,
            ...
            foreach ($placeholders as $placeholder) {
                switch ($placeholder) {
                    ...
                    case 'emp_join':
                        if ($emp_join_fkey == 0) {
                            // when no employee join selected — placeholders remain as text
                            $employee_join_name = '{:employee_join_name}';
                            $employee_join_avatar = '{:employee_join_avatar}';
                            $employee_join_mobile_no = '{:employee_join_mobile_no}';
                            $employee_join_country = '{:employee_join_nationality}';
                            $employee_join_date_of_birth = '{:employee_join_date_of_birth}';
                            $employee_join_email = '{:employee_join_email}';
                            $employee_join_address = '{:employee_join_address}';
                            $employee_join_pincode = '{:employee_join_pincode}';
                            $employee_join_district = '{:employee_join_district}';
                            $employee_join_guardian = '{:employee_join_guardian}';
                            $employee_join_gender = '{:employee_join_gender}';
                            $employee_join_state = '{:employee_join_state}';
                            $employee_join_bank_name = '{:employee_join_bank_name}';
                            $employee_join_branch_name = '{:employee_join_branch_name}';
                            $employee_join_ifsc_code = '{:employee_join_ifsc_code}';
                            $employee_join_account_no = '{:employee_join_account_no}';
                            $employee_join_pf = '{:employee_join_pf}';
                            $employee_join_previous_member = '{:employee_join_previous_member}';
                            $employee_join_esi_dispensary = '{:employee_join_esi_dispensary}';
                            $employee_join_esi = '{:employee_join_esi}';
                            $employee_join_eps = '{:employee_join_eps}';
                            $employee_join_pan_no = '{:employee_join_pan_no}';
                            $employee_join_uan = '{:employee_join_uan}';
                            $employee_join_relation_guardian = '{:employee_join_relation_guardian}';
                            $employee_join_maritual_status = '{:employee_join_maritual_status}';
                            $employee_join_wps_code = '{:employee_join_wps_code}';
                            $employee_join_lwf_code = '{:employee_join_lwf_code}';
                            $employee_join_blood_group = '{:employee_join_blood_group}';
                            $employee_join_issue_date = '{:employee_join_issued_date}';
                        } else {
                            $emp_join = $this->getEmployeesJoin($emp_join_fkey);
                            $emp_join = isset($emp_join[0]) ? $emp_join[0] : [];
                            $this->loadModel('Country'); // ensure model loaded
                            $country_id = isset($emp_join['nationality']) ? $emp_join['nationality'] : null;
                            $country_code = '';

                            if (!empty($country_id)) {
                                $country = $this->Country->find('first', [
                                    'conditions' => ['Country.id_countries' => $country_id],
                                    'fields' => ['Country.name']
                                ]);

                                if (!empty($country)) {
                                    $country_code = $country['Country']['name'];
                                }
                            }
                            ...
                            if (!empty($emp_join)) {
                                $employee_join_name = $emp_join['emp_name'];
                                $employee_join_avatar = !empty($emp_join['avatar'])
                                    ? '<img src="' . htmlspecialchars($emp_join['avatar'], ENT_QUOTES, 'UTF-8') . '" alt="Employee Image" style="width: 100px; height: 120px;">'
                                    : '';

                                $employee_join_mobile_no = $emp_join['mobile_no'];
                                $employee_join_country = $country_code;
                                $employee_join_date_of_birth = date("d-m-Y", strtotime($emp_join['dob']));
                                $employee_join_email = $emp_join['email'];
                                $employee_join_address = $emp_join['address'];
                                $employee_join_pincode = $emp_join['pincode'];
                                $employee_join_district = $emp_join['district'];
                                $employee_join_guardian = $emp_join['guardian'];
                                $employee_join_gender = $emp_join['gender'];
                                $employee_join_state = $emp_join['state'];
                                $employee_join_bank_name = $emp_join['bank_name'];
                                $employee_join_branch_name = $emp_join['branch_name'];
                                $employee_join_ifsc_code = $emp_join['ifsc_code'];
                                $employee_join_account_no = $emp_join['account_no'];
                                $employee_join_pf = $emp_join['pf'];
                                $employee_join_previous_member = $emp_join['previous_member'];
                                $employee_join_esi_dispensary = $emp_join['esi_dispensary'];
                                $employee_join_esi = $emp_join['esi'];
                                $employee_join_eps = $emp_join['eps'];
                                $employee_join_pan_no = $emp_join['pan_no'];
                                $employee_join_uan = $emp_join['uan'];
                                $employee_join_relation_guardian = $emp_join['relation_guardian'];
                                $employee_join_maritual_status = $emp_join['maritual_status'];
                                $employee_join_wps_code = $emp_join['wps_code'];
                                $employee_join_lwf_code = $emp_join['lwf_code'];
                                $employee_join_blood_group = $emp_join['blood'];
                                $employee_join_issue_date = date('d-m-Y');
                            }
                        }
                        break;


                    // end// edited by bindu 25-10-25 end
                    case 'comp_contact':
                        if ($company_fkey == 0) {
            ...
            $replace = array(
                ...
                // edited by bindu 25-10-25
                $employee_join_name,
                $employee_join_avatar,
                $employee_join_mobile_no,
                $employee_join_country,
                $employee_join_date_of_birth,
                $employee_join_email,
                $employee_join_address,
                $employee_join_pincode,
                $employee_join_district,
                $employee_join_guardian,
                $employee_join_gender,
                $employee_join_state,
                $employee_join_bank_name,
                $employee_join_branch_name,
                $employee_join_ifsc_code,
                $employee_join_account_no,
                $employee_join_pf,
                $employee_join_previous_member,
                $employee_join_esi_dispensary,
                $employee_join_esi,
                $employee_join_eps,
                $employee_join_pan_no,
                $employee_join_uan,
                $employee_join_relation_guardian,
                $employee_join_maritual_status,
                $employee_join_wps_code,
                $employee_join_lwf_code,
                $employee_join_blood_group,
                $employee_join_issue_date,
                // end
                $company_name,
                $company_nature,
                $company_type,
            ...
            // Only wrap with header/footer for actual PDF generation, NOT for editor preview (preview==3).
            // When preview==3, the HTML is returned to CKEditor via JSON and then saved to the DB.
            // If header/footer were included here, they would be embedded in the saved content and
            // then added AGAIN by the PDF download branch — causing them to appear twice.
            if ($preview != 3) {
                $prev_header_h = !empty($preview_header_image) ? 100 : 15;
                $prev_footer_h = !empty($preview_footer_image) ? 90 : 15;
                $previewWrapped = '';
                $previewWrapped .= '<style>
                    @page {
                        margin-top: ' . $prev_header_h . 'px;
                        margin-bottom: ' . $prev_footer_h . 'px;
                        margin-left: 25px;
                        margin-right: 25px;
                    }
                    #doc-page-header {
                        position: fixed;
                        top: -' . $prev_header_h . 'px;
                        left: -25px;
                        right: -25px;
                        height: ' . $prev_header_h . 'px;
                    }
                    #doc-page-header img { width: 100%; height: 100%; display: block; }
                    #doc-page-footer {
                        position: fixed;
                        bottom: -' . $prev_footer_h . 'px;
                        left: -25px;
                        right: -25px;
                        height: ' . $prev_footer_h . 'px;
                    }
                    #doc-page-footer img { width: 100%; height: 100%; display: block; }
                    #footer { position: fixed; left: 0; bottom: 5px; right: 0; height: 20px; text-align: center; padding: 10px; font-size: 12px; }
                </style>';
                if (!empty($preview_header_image)) {
                    $previewWrapped .= '<div id="doc-page-header"><img src="' . htmlspecialchars($preview_header_image, ENT_QUOTES, 'UTF-8') . '" alt="Header"></div>';
                }
                if (!empty($preview_footer_image)) {
                    $previewWrapped .= '<div id="doc-page-footer"><img src="' . htmlspecialchars($preview_footer_image, ENT_QUOTES, 'UTF-8') . '" alt="Footer"></div>';
                }
                $previewWrapped .= $html;
                $html = $previewWrapped;
            }
        ...
        /* Clear any buffered output so it doesn't corrupt the PDF stream */
        while (ob_get_level()) {
            ob_end_clean();
        }
    ...
    public function getPlaceholder()
    {
        ...
        return array(
            ...
            '{:salary_break_up}',
            //End
            // emp_join placeholders // edited by bindu 25-10-25
            '{:employee_join_name}',
            '{:employee_join_avatar}',
            '{:employee_join_mobile_no}',
            '{:employee_join_nationality}',
            '{:employee_join_date_of_birth}',
            '{:employee_join_email}',
            '{:employee_join_address}',
            '{:employee_join_pincode}',
    ...
    public function saveDocument()
    {
        ...
        $data['emp_join_fkey'] = isset($_POST['employee_join']) ? $_POST['employee_join'] : 0; // edited by bindu 25-10-25
        ...
        if ($empname == '' && !empty($data['emp_join_fkey'])) {

            $empJoin = $this->Documents->query(
        ...
        WHERE emp_join_pkey = " . (int) $data['emp_join_fkey']

            );

            if (!empty($empJoin)) {

                $empname = $empJoin[0]['emp_join']['first_name'];
            }
        }
        if ($empname == '') {
    ...
    public function getEmployeesJoin($emp_join_pkey = '')
    {
        // Build condition dynamically
        $condition = ($emp_join_pkey != '') ? 'AND emp_join_pkey = ' . (int) $emp_join_pkey : '';

        // Use the correct database connection
        $this->Documents->useDbConfig = $this->Session->read('ds');

        // Query the emp_join table for active records
        $result = $this->Documents->query("
        SELECT emp_join_pkey,
               first_name AS emp_name,
               profile_image_url AS avatar,
               mobile_no,
               nationality_id AS nationality,
               date_of_birth AS dob,
               email,
               address,
               pincode,
               district,
               guradian AS guardian,
               classification AS gender,
               state,
               bank AS bank_name,
               bank_branch AS branch_name,
               ifsc_code,
               account_no,
               pf,
               previous_member_id AS previous_member,
               esi_dispensary,
               esi,
               eps,  
               pan_no,
        ...
        ORDER BY emp_join_pkey DESC
    ");
        // Normalize CakePHP query result
        $employeejoin = [];
        foreach ($result as $val) {
            if (isset($val[0])) {
                $employeejoin[] = $val[0];
            } elseif (isset($val['emp_join'])) {
                $employeejoin[] = $val['emp_join'];
            }
        }
        ...
        foreach ($placeholders as $placeholder) {
            switch ($placeholder) {
                ...
                case 'comp_contact':
                    ...
                    break;
                case 'emp_join': // edited by bindu 25-10-25
                    $result[$placeholder] = $this->getEmployeesJoin();
                    break; // edited by bindu 25-10-25 end
                case 'branch':
    ...
    public function savefile()
    {
        ...
        try {
            if (!$file_key || !isset($_FILES[$file_key]['error']) || is_array($_FILES[$file_key]['error'])) {
            ...
            if ($_FILES[$file_key]['size'] > 1000000) {
                $resp['error'] = 'Exceeded filesize limit.';
                echo json_encode($resp);
                return;
            }
            ...
            $finfo = new finfo(FILEINFO_MIME_TYPE);
            $ext = array_search(
                $finfo->file($_FILES[$file_key]['tmp_name']),
                array(
                    'jpg' => 'image/jpeg',
                    'jpeg' => 'image/jpeg',
                    'png' => 'image/png',
            ...
            if ($file_key === 'avatarfile') {
                $target_dir = WWW_ROOT . "img/";
                if (!file_exists($target_dir)) {
                    mkdir($target_dir, 0755, TRUE);
                }

                $filename = sprintf($target_dir . '%s.%s', sha1_file($_FILES[$file_key]['tmp_name']), $ext);
                $dir = $this->webroot . sprintf('img/%s.%s', sha1_file($_FILES[$file_key]['tmp_name']), $ext);
                ...
                if (!move_uploaded_file($_FILES[$file_key]['tmp_name'], $filename)) {
                    ...
                    $resp['data'] = $data;
                    $this->Documents->useDbConfig = $this->Session->read('ds');
                    $this->Documents->query("insert into doc_images(image_url,company_code,created_by) values('$url','$companycode','$user_id')");
                }
            } else {
                // edited by bindu 27-05-2026
                $target_dir = "./img/";
                ...
                $filename = sprintf($target_dir . '%s.%s', sha1_file($_FILES[$file_key]['tmp_name']), $ext);
                // edited by bindu 27-05-2026
                $dir = 'img/' . sprintf('%s.%s', sha1_file($_FILES[$file_key]['tmp_name']), $ext);

                if (!move_uploaded_file($_FILES[$file_key]['tmp_name'], $filename)) {
                    $resp['error'] = 'Failed to move uploaded file to destination.';
                } else {
                    $resp['data']['documents'] = $dir;
                }
            }
        } catch (RuntimeException $e) {
            $resp['error'] = 'Runtime exception occurred.';
        }
    ...
    public function convertimage()
    {
        ...
        $data = array(
            ...
            'url' => 'https://v1.mypayrollmaster.online/DocumentManagers/renderImageTempalte'
        );
    ...
    public function printPreview($document_id = '')
    {
        ...
        if (!empty($document_id)) {
            $data = $this->Documents->query("SELECT d.document_name, d.document, d.creation_date, d.created_by, d.doc_id, dt.header_image, dt.footer_image FROM documents d LEFT JOIN doc_template dt ON dt.template_pkey = d.template_fkey AND dt.status = 1 WHERE d.document_pkey = " . intval($document_id));
            if (!empty($data)) {
                ...
                $print_header_image = isset($data[0]['doc_template']['header_image']) ? $data[0]['doc_template']['header_image'] : '';
                $print_footer_image = isset($data[0]['doc_template']['footer_image']) ? $data[0]['doc_template']['footer_image'] : '';
                $print_header_path = $print_header_image;
                if (!empty($this->webroot) && $this->webroot !== '/' && strpos($print_header_path, $this->webroot) === 0) {
                    $print_header_path = substr($print_header_path, strlen($this->webroot));
                }
                $print_header_image_fs = !empty($print_header_image) ? 'file://' . WWW_ROOT . ltrim($print_header_path, '/') : '';

                $print_footer_path = $print_footer_image;
                if (!empty($this->webroot) && $this->webroot !== '/' && strpos($print_footer_path, $this->webroot) === 0) {
                    $print_footer_path = substr($print_footer_path, strlen($this->webroot));
                }
                $print_footer_image_fs = !empty($print_footer_image) ? 'file://' . WWW_ROOT . ltrim($print_footer_path, '/') : '';
                $printWrapped = '';
                if (!empty($print_header_image_fs)) {
                    $printWrapped .= '<img src="' . htmlspecialchars($print_header_image_fs, ENT_QUOTES, 'UTF-8') . '" style="width:100%;display:block;" alt="Header">';
                }
                $printWrapped .= $html;
                if (!empty($print_footer_image_fs)) {
                    $printWrapped .= '<img src="' . htmlspecialchars($print_footer_image_fs, ENT_QUOTES, 'UTF-8') . '" style="width:100%;display:block;" alt="Footer">';
                }
                $html = $printWrapped;
                $html .= '<style>
                #footer {
