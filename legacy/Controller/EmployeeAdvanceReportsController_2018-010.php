...
class EmployeeAdvanceReportsController extends AppController {
     ...
     * @var array
     */
    public $uses = array('EditPunches', 'Attendance', 'CentralControl', 'UserCredentials', 'EmployeeDetails', 'EmployeeProfessionalDetails', 'DeviceAttendance', 'Departments', 'Grades', 'Verticals', 'Units', 'ReportCriterias', 'AttendanceRegister', 'AttendanceRegisterReport', 'DbConfig', 'MobileUserauditor',
        'CompanyContactInfo', 'EmployeeAdvance'); //santhu
...
//debug($mode);
    ...
    private function generateemployeeadvance($mode = '') {
        ...
        $arr_employee_advance = $this->EmployeeAdvance->query("select au.*,ei.*,EmployeeDetails.branch_code,Units.branch_name
        ...
        //debug($arr_employee_advance);
        $arr_emp_advance_template = array();
        if ($needBranchWiseReport) {
            //Parse array for branchwise report
            foreach ($arr_employee_advance as $employee_advance) {
                // debug($employee_advance);
                ...
                //debug($branch_name);
                if ($branch_code != '') {
                    if (!isset($arr_emp_advance_template[$branch_code])) {
                        $arr_emp_advance_template[$branch_code] = array(
                    ...
                    $arr_emp_advance_template[$branch_code]['employeeadvance'][] = $request;
                    //debug($arr_emp_advance_template);
                }
            }
        } else {
            //Parse array for simple report
            $arr_emp_advance_template['employeeadvance'] = array();
            foreach ($arr_employee_advance as $employee_advance) {
                $request = array();
                $request['emp_name'] = isset($employee_advance['ei']['EmpName']) ? $employee_advance['ei']['EmpName'] : '';
                $request['employee_id'] = isset($employee_advance['ei']['employee_id']) ? $employee_advance['ei']['employee_id'] : '';
                ...
                $arr_emp_advance_template['employeeadvance'][] = $request;
            }
        }

        $this->set('needBranchWiseReport', $needBranchWiseReport);
        $this->set('arr_emp_advance_template', $arr_emp_advance_template);
        ...
        switch ($mode) {
            ...
            case 'excel' :
                ...
                $file_name = isset($str_company_code) ? $str_company_code . "Emp_Advance_report.xlsx" : "ShiftPolicy" . strtotime() . ".xlsx";
                ...
                $worksheet->setCellValueByColumnAndRow(0, 1, "Employee Advance Report");
                ...
                if (isset($needBranchWiseReport) && $needBranchWiseReport == true) {
                    foreach ($arr_emp_advance_template as $branch_code => $emp_advance) {
...
//                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 5), $rowcount)->getFont()->setBold(true);
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 5), $rowcount, 'Advance Amount');
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 5), $rowcount)->getFont()->setBold(true);
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 6), $rowcount, 'Affected Month');
                        $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 6), $rowcount)->getFont()->setBold(true);
                        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 7), $rowcount, 'Created Date');
...
//                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 5), ($rowcount), $des);
                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 5), $rowcount, $amt);
                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 6), $rowcount, $month);
                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 7), $rowcount, $create);
                                $rowcount = $rowcount + 1;
                                $k++;
                            }
                        } else {
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 5), $rowcount, 'Branch');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 5), $rowcount)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 6), $rowcount, 'Advance Amount');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 6), $rowcount)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 7), $rowcount, 'Affected Month');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 7), $rowcount)->getFont()->setBold(true);
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 8), $rowcount, 'Created Date');
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(($columncount + 8), $rowcount)->getFont()->setBold(true);
                    $rowcount = $rowcount + 1;


                    $arr_data = $arr_emp_advance_template['employeeadvance'];
                    if (count($arr_data) >= 0) {
                        $k = 1;
                        foreach ($arr_data as $val) {
                            $name = $val['emp_name'];
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 7), $rowcount, $month);
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($columncount + 8), $rowcount, $create);
                            $rowcount = $rowcount + 1;
                            $k++;
                        }
                    } else {
                ...
                $objPHPExcel->getActiveSheet()->setTitle('Employee Advance Report');
                /* header footer */
