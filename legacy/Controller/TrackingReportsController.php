 ...
 */
class TrackingReportsController extends AppController
{
        ...
        if (isset($model) && $model != '') {
            ...
            if ($model == 'Departments') {
                ...
                if ($user_group == 2 && !$is_addon) {
                    ...
                    $cur_emp_branch_find = $this->EmployeeDetails->find("first", array("fields" => "branch_code", "conditions" => array("emp_pkey" => $emp_fkey, "status" => 1)));
                    if (!empty($cur_emp_branch_find)) {
                        $cur_emp_branch = $cur_emp_branch_find['EmployeeDetails']['branch_code'];
                        $conditions = array("status" => 1, "branch_code" => $cur_emp_branch);
                    } else {
                        $conditions = array("status" => 1);
                    }
                } else {
            ...
            switch ($model) {
                ...
                case 'EmployeeDetails':
                    ...
                    if ($user_group == 2 && !$is_addon) {
                        $emp_fkey = $this->Session->read('emp_fkey');
                        $conditions[] = array("EmployeeDetails.emp_pkey" => $emp_fkey);
                    } else if ($user_group == 2 && $is_addon) {
                        $emp_fkey = $this->Session->read('emp_fkey');
                        $useracess = $this->EmployeeDetails->query("select * from user_access as Useraccess where user_fkey = '$emp_fkey' and menu_id = '0' and active = 'Y'");
                        $access = isset($useracess['0']['Useraccess']['active']) ? $useracess['0']['Useraccess']['active'] : '';
                        if ($access == 'Y') {
                            $cur_emp_branch_find = $this->EmployeeDetails->find("all", array("fields" => "branch_code", "conditions" => array("emp_pkey" => $emp_fkey, "status" => 1)));
                            $cur_emp_branch = $cur_emp_branch_find[0]['EmployeeDetails']['branch_code'];
                            $conditions[] = array(
                                "OR" => array(
                                    "EmployeeDetails.branch_code" => $cur_emp_branch,
                                    "EmployeeProfessionalDetails.attr1" => $emp_fkey,
                                    "EmployeeDetails.emp_pkey" => $emp_fkey,
                                )
                            );
                        } else {
                            $conditions[] = array("OR" => array("EmployeeDetails.emp_pkey" => $emp_fkey, "EmployeeProfessionalDetails.attr1" => $emp_fkey));
                        }
                    }
	 ...
	 BETWEEN DATE_FORMAT('$from_dates','%Y-%m-%d %T') AND DATE_FORMAT('$to_dates','%Y-%m-%d %T') order by created_time ) MO) XX
         WHERE INVAL = 'in' AND OUTVAL IN ('out' ) order by INTIME");
            ...
            $emplist = $this->MobileUserauditor->query("SELECT distinct(EMP_PKEY) as emp FROM (SELECT MO.C1 AS OUTVAL,MO.EMP_PKEY,MO.purpose,MO.employee_id,MO.branch,
                                                        ...
                                                        FROM (SELECT @PARTITION_BY_COLUMN = NULL, @S := NULL, @C := NULL, @LOGIN := NULL, @INVAL := NULL ) 
                                                        vars, mob_user_locations att, employee_info ed,user_credentials uc where att.user_id= uc.user_id
                                                        and uc.emp_fkey=ed.emp_pkey  and ed.branch_code= '$branch'
							and date_format(att.created_time,'%Y-%m-%d %T') 
                                                        BETWEEN DATE_FORMAT('$from_dates','%Y-%m-%d %T')
                                                        AND DATE_FORMAT('$to_dates','%Y-%m-%d %T')
                                                        order by created_time ) MO) XX
            ...
            foreach ($arr_leavepolicygroupids as $leavepolicygroupid) {
                ...
                if ($arr_form_data['select-criteria1'] == 'EmployeeDetails') {
                    ...
                    $arr_leavepolicy_details = $this->DeviceAttendance->query("select cus_visit_locations_pkey,start_latitude,start_longitude,stepin_latitude,stepin_longitude,stepout_latitude,stepout_longitude,"
                        ...
                        . " from cus_visit_locations "
                        . "left join employee_info on (cus_visit_locations.emp_fkey = employee_info.emp_pkey) "
                        . "where branch_code = '$leavepolicygroupid' and stepin_time "
                        . "BETWEEN '$start_month' AND '$end_month' group by stepin_time");
                }
