...
class ProjectController extends AppController {
	...
	$this->SiteMaster->useDbConfig = $this->Session->read('ds');
        ...
        $this->layout = NULL;
        $site = $this->SiteMaster->query("select site_pkey,site_id,site_name from site where status = 1");
        $this->set('site',$site); 
    ...
    public function activity_save(){
        ...
        $project = $arr_form_data['project_fkey'] = $arr_form_data['project'];
        $work_date = $arr_form_data['date'] = $arr_form_data['work_date']; 
        $work_detail = $arr_form_data['work_detail'];
        $activity_pkey = $arr_form_data['activity_pkey'];
        $int_datecount = 0;
        if ($work_date !='') {
        ...
        if($int_datecount==0){
            ...
            $arr_form_data['activity_pkey']= $activity_pkey;
            $result = $this->ProjectActivity->save($arr_form_data);
            $resp = array();
            $resp["success"] = true;
...
//        $branch[] = array("id" => "0", "text" => "ALL");
    ...
    public function form($site_pkey = 0) {
        ...
        if ($site_pkey != 0) {
            $arr_sites = $this->EmployeeDetails->query("select site.*,contacts.*,concat(emp_details.first_name,' ',ifnull(emp_details.last_name,'')) as manager from site left join emp_details on (emp_details.emp_pkey = site.user_pkey) left join contacts on (contacts.contact_id = site.contact_name) where site_pkey = '$site_pkey' ");
            $this->set('arr_sites', $arr_sites);
            $arr_engineer = $this->EmployeeDetails->query("select concat(emp_details.first_name,' ',ifnull(emp_details.last_name,'')) as engineer from site left join emp_details on (emp_details.emp_pkey  = site.contact_name) where site_pkey = '$site_pkey' ");
            $this->set('arr_engineer', $arr_engineer);
			//debug($arr_engineer);
...
where site_fkey = '$site_pkey'  and site_transactions.status = '1'  ");
 ...
 public function delete($site_pkey = 0)
    {
		$this->SiteMaster->useDbConfig = $this->Session->read('ds');
        $this->autoRender = FALSE ; 
        if($site_pkey ==0){
        ...
        if ($site_pkey != 0) {
            $this->SiteMaster->updateAll(array('status' => 0), array('site_pkey' => $site_pkey));
            echo json_encode(array('msg' => 'Project details deleted successfully!'));
        } else {
            echo json_encode(array('msg' => 'Project details deletion failed!'));
        }     
    }
 ...
 from emp_menu EmployeeMenu  left join user_access as u on(u.menu_id = EmployeeMenu.menu_id) and u.user_fkey='$emp_pkey' and u.active='Y' where EmployeeMenu.parent_id = 0 and EmployeeMenu.active = 'Y'");
    ...
    public function lists($param = "") {
        ...
        $arr_useraccess = $this->Useraccess->query("select (select concat(emp_details.first_name,' ',ifnull(emp_details.last_name,''))  from emp_details where emp_pkey = site.contact_name  )as engineer,
(select concat(emp_details.first_name,' ',ifnull(emp_details.last_name,''))  from emp_details where emp_pkey = site.user_pkey )as manager,
site.site_name,site.site_id,site.site_pkey,site.customer_name,site.latitude,site.po_expirydate,site.longitude,site.special_remarks,site.branch_code from site"
                . " where status = '1' $cond  ORDER BY site.site_pkey DESC limit $limit offset $offset ");
        ...
        $totalcount = $this->Useraccess->query("SELECT count(*) as cnt FROM `site` where status = '1' $cond  ORDER BY site.site_pkey ");

        $rows = array();
        foreach ($arr_useraccess as $key => $val) {
           //$rows[] = $val['site'];
           $val['site']['po_expirydate'] = isset($val['site']["po_expirydate"]) ? date('d-m-Y',strtotime($val['site']["po_expirydate"])) : '';
