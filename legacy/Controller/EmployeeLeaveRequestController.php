 ...
 */
class EmployeeLeaveRequestController extends AppController
{
    ...
    public $name = 'EmployeeLeaveRequest';
    public $datatable;
    ...
    public function employeeleaves()
    {
        ...
        //On 10 Aug 2015
        $emp_leave_count = $this->LeaveRequests->find(
            'count',
            array(
                'conditions' => array(
                    'OR' => array(
    ...
    public function getusers()
    {
        ...
        //Edited by Akshay on 18-2-2024
        $emp_fkey = isset($arr_request_data['empPkey']) ? $arr_request_data['empPkey'] : '';
    ...
    public function manageempleave($leaveentryId = 0)
    {
            ...
            if ($this->checkIfLeaveRequestEditable($leaveentryId)) {
                ...
                try {
                    ...
                    $arr_leave_details = $this->LeaveRequests->find("first", array(
                        'fields' => 'EmployeeInformation.*,TODATE,FROMDATE,ISAutherized,ISAPPROVED,APPROVED_date,ISAutherizedby,APPROVEDBY,salary_head_item_fkey,EMP_fkey,LEAVESTATUS,SalaryHeadItems.item, leave_days, file_name',
                    ...
                    // debug($arr_leave_details); exit;
                    $fromdate = isset($arr_leave_details['LeaveRequests']['FROMDATE']) ? $arr_leave_details['LeaveRequests']['FROMDATE'] : '';
                    $todate = isset($arr_leave_details['LeaveRequests']['TODATE']) ? $arr_leave_details['LeaveRequests']['TODATE'] : '';
                    ...
                    $salary_head_item_disc = isset($arr_leave_details['SalaryHeadItems']['item']) ? $arr_leave_details['SalaryHeadItems']['item'] : ''; // Edited by Akshay on 1-11-2023
                    $leavedays = isset($arr_leave_details['LeaveRequests']['leave_days']) ? $arr_leave_details['LeaveRequests']['leave_days'] : 0;
                    $file_name = isset($arr_leave_details['LeaveRequests']['file_name']) ? $arr_leave_details['LeaveRequests']['file_name'] : '';
                    ...
                    if (($fromdate >= $att_startdate1) && ($todate <= $att_enddate1)) {
                        ...
                        $arr_attendance_register = $this->LeaveRequests->query("select count(*) cnt from attendance_register where month_year=date_format('$yearmonth','%Y-%m') and isdelete='N' and emp_fkey= '$emp_fkey' ");
                        $count = $arr_attendance_register['0']['0']['cnt'];
                    } else if (($fromdate <= $att_enddate1) && ($todate >= $att_enddate1)) {
                        $yearmonth = date('Y-m-d', strtotime($att_enddate1));
                        $arr_attendance_register = $this->LeaveRequests->query("select count(*) cnt from attendance_register where month_year=date_format('$fromdate','%Y-%m') and isdelete='N' and emp_fkey= '$emp_fkey' ");
                        $yearmonth1 = date('Y-m-d', strtotime($fromdate . ' + 1 months'));
                        $arr_attendance_register1 = $this->LeaveRequests->query("select count(*) cnt from attendance_register where month_year=date_format('$yearmonth1','%Y-%m') and isdelete='N' and emp_fkey= '$emp_fkey' ");
                    ...
                    try {
                        if ($LEAVESTATUS == 'CancellationOfApproved' || $LEAVESTATUS == 'CancellationOfAuthorized') {
                            ...
                            $responsedata = $this->criterias($empfkey, $fromdate, $to_date);
                        }
                    ...
                    $to_month = date("Y-m-d",  strtotime($month));
                    if ($allow_negative == 'Y') {
                    ...
                    //Edited by Akshay on 1-11-2023
                    $warningmsg = '';
                    if (($salary_head_item_fkey == 86 ||  trim($salary_head_item_disc) == 'Sick Leave') && $leavedays >= 2 && (trim($file_name) == '' || $file_name == NULL)) {
                        $warningmsg .= 'Document not uploaded. It is required for taking two or more sick leaves.';
                    }
                ...
                $this->EmployeeInfo->useDbConfig = $this->Session->read('ds');
                $arr_leave_details = $this->LeaveRequests->find("first", array(
                    'fields' => 'EmployeeInformation.*,TODATE,FROMDATE,ISAutherized,ISAPPROVED,APPROVED_date,ISAutherizedby,APPROVEDBY,salary_head_item_fkey,EMP_fkey,LEAVESTATUS,file_name,file_type,SalaryHeadItems.item, leave_days',
                    'conditions' => array('LEAVEENTRYID' => $leaveentryId),
                    'joins' => array(
                        array(
                            'table' => 'employee_info',
                ...
                $authorized_name = $this->EmployeeDetails->find("all", array("conditions" => array("emp_pkey" => $authorized)));
                $this->set("authorized_name", $authorized_name);
                $arr_leave_details['authorized_name'] = isset($authorized_name['0']['EmployeeDetails']['first_name']) ? $authorized_name['0']['EmployeeDetails']['first_name'] . ' ' . $authorized_name['0']['EmployeeDetails']['last_name'] : '';
                $this->LeavePolicy->useDbConfig = $this->Session->read('ds');
                $arr_leave_policy = $this->LeavePolicy->find(
                    "all",
                    array(
        ...
        $company_code = strtolower($this->Session->read('company_code'));
        $this->set('company_code', $company_code);
        $path = "https://mypayrollmaster.online/leaveupload/" . strtolower($company_code) . "/";
        $this->set('path', $path);
   ...
   public function listleaves()
    {
        ...
        $cur_emp_key = $this->Session->read("emp_fkey");
        try {
            $fields = 'ISAutherizedby,APPROVEDBY,LEAVEENTRYID,SalaryHeadItems.item as leave_type,applied_date,FROMDATE,FROMHALF,TODATE,TOHALF,LEAVESTATUS,leave_days,ei.EmpName,ei.employee_id';
            $joins = array(
                array(
                    'table' => 'salary_head_items',
                    'alias' => 'SalaryHeadItems',
                ...
                array(
                    ...
                    'type' => 'LEFT',
                    'foreignKey' => false,
                    'conditions' => array('LeaveRequests.EMP_fkey = ei.emp_pkey'),
                    'order' => array('ei.emp_name ASC') //Edited by Akshay on 12-11-2024
                )
            );
            ...
            //  $count = $this->LeaveRequests->find("count");
            // $count = $this->LeaveRequests->find()->count();
            $conditions = array();

            if ($emp != '') {
                $conditions['ei.EmpName LIKE'] = '%' . $emp . '%';
            }
            try {
            ...
            //edited by sinsiya 06-03-2024
            $arr_myleaverequestscount = $this->LeaveRequests->find("all", array(
                'fields' => $fields,
                'joins' => $joins,
                'order' => array('LEAVEENTRYID DESC'),
                'conditions' => $conditions
            ));
            $count = count($arr_myleaverequestscount);
        } catch (Exception $ex) {
            $resp["success"] = false;
        ...
        foreach ($arr_myleaverequests as $key => $value) {
            ...
            $leavePkey = $value["LeaveRequests"]['LEAVEENTRYID'];
            $leaveStatus = $value["LeaveRequests"]['LEAVESTATUS'];
            if ($leaveStatus == 'Applied' || $leaveStatus == 'Approved' || $leaveStatus == 'Authorized') {
                $resp_myleaverequests["rows"][$key]['doc'] = "<a href='javascript:void(0)' onclick=\"printleave('{$leavePkey}')\"><button>Download</button></a>";
            } else {
                $resp_myleaverequests["rows"][$key]['doc'] = "<a href='javascript:void(0)' onclick=\"printleave('{$leavePkey}')\" disabled><button disabled>Download</button></a>";
            }

            $resp_myleaverequests["rows"][$key]['empname'] = $value['ei']['EmpName'] . '-' . $value['ei']['employee_id'];
            // $resp_myleaverequests["rows"][$key]['empname'] = $value['ei']['EmpName'];
            // $resp_myleaverequests["rows"][$key]['empid'] = $value['ei']['employee_id'];
        }
    ...
    //    $html2pdf->Output('leave.pdf', 'D');

    //   $this->render('leave');

    //}
    public function printleave($pkey = 0)
    {
        $this->autoRender = false;
        ...
        try {
            $fields = 'ISAutherizedby,APPROVEDBY,LEAVEENTRYID,SalaryHeadItems.salary_head_item_pkey,SalaryHeadItems.item as leave_type,SalaryHeadItems.occurance as leavetype,applied_date,FROMDATE,FROMHALF,TODATE,TOHALF,LEAVESTATUS,leave_days,Reason,EMP_fkey,ei.EmpName,ei.employee_id,ai.EmpName AS AutherizedName,ap.EmpName AS ApprovedName';
            $joins = array(
                array(
                    'table' => 'salary_head_items',
                    'alias' => 'SalaryHeadItems',
                ...
                array(
                    ...
                    'type' => 'LEFT',
                    'foreignKey' => false,
                    'conditions' => array('LeaveRequests.ISAutherizedby = ai.emp_pkey')
                ),
                array(
                    'table' => 'employee_info',
                    'alias' => 'ap',
                    'type' => 'LEFT',
                    'foreignKey' => false,
                    'conditions' => array('LeaveRequests.APPROVEDBY = ap.emp_pkey')
                )
            );
            $conditions = array('LEAVEENTRYID' => $pkey);
            ...
            // $salaryheaditempkey =$arr_myleaverequests[0]['SalaryHeadItems']['salary_head_item_pkey']; 
            // $empkey=$arr_myleaverequests[0]['LeaveRequests']['EMP_fkey'];
            // debug($empkey);
            //exit;
            //  $from = date('Y', strtotime($arr_myleaverequests[0]['LeaveRequests']['FROMDATE'])); 
            //  $toDate = $arr_myleaverequests[0]['LeaveRequests']['TODATE'];
            // debug($empkey);
            // debug($salaryheaditempkey);
            //debug($from);
            //   $lbalance = $this->LeaveRequests->query("(SELECT leave_balance_inthe_year_fn('".$empkey."','".$salaryheaditempkey."', '" . $from . "') leavebalance,
            //     leave_taken_fn('".$empkey."','".$salaryheaditempkey."','" . $from . "') leavetaken)");
            //$get_finyear = $this->LeaveRequests->query("select fin_year  from fin_year where lcase(Year_status)='open' and vattr1 = 0 and is_current_finyear='Y' and status=1
            // and branch_code= (select branch_code from emp_details where emp_Pkey='$empkey' ) ORDER BY fin_year DESC LIMIT 1  "); // removed this line code - and '$end_date' between start_month and end_month
            //$finyear = isset($get_finyear['0']['fin_year']['fin_year']) ? $get_finyear['0']['fin_year']['fin_year'] : date('Y');
            //debug($finyear);
            //debug($from);
            // $months =$toDate;
            ...
            // $lmonthbalance = $this->LeaveRequests->query("select leave_balance_inthe_month_fn('$empkey','$salaryheaditempkey','$months','$finyear') as LeaveBalancemonth,leave_taken_fn('".$empkey."','".$salaryheaditempkey."','" . $finyear . "') leavetaken");
            // debug($lmonthbalance);
            ...
            //debug("(SELECT leave_balance_inthe_year_fn('".$empkey."','".$salaryheaditempkey."', '" . $from . "') leavebalance,
            // leave_taken_fn('".$empkey."','".$salaryheaditempkey."','" . $from . "') leavetaken)");
            //exit;
            // debug($toDate);
            // exit;
            ...
            $this->set('arr_myleaverequests', $arr_myleaverequests);


            $this->set('mode', 'pdf');
            $view = new View($this, false);
            $view_output = trim($view->render('leave'));
            // echo $view_output;
            ...
            $html2pdf->pdf->SetDisplayMode('fullpage');
            $html2pdf->writeHTML($view_output);
            $html2pdf->Output('leavepass.pdf', 'D');
            $this->render('leave');
            //}
        } catch (Exception $e) {
            debug($e);
    ...
    public function addeditleave($leaveentryId = 0)
    {
        ...
        try{
            $arr_users = $this->LeaveRequests->query("
            SELECT 
                CONCAT(emp_details.first_name, ' ', IFNULL(emp_details.last_name, '')) AS name,
            ...
            FROM 
                emp_details
            LEFT JOIN 
                employee_info ei ON ei.emp_pkey = emp_details.emp_pkey
            WHERE 
                emp_details.status = '1' 
                AND emp_details.emp_pkey NOT IN ('$emp_fkey')
            ORDER BY 
                name
        ");
        }catch(Exception $e){
    ...
    public function Getauther_approv()
    {

        $this->autoRender = false;
        $arr_form_data = $this->request->data;
        $emp_key = $arr_form_data['employee_pkey'];
        $company_code = $this->Session->read('company_code');
        $this->EmployeeDetails->useDbConfig = $this->Session->read('ds');
        //debug($emp_key);
        //debug($company_code);

        $auth = "select `leave_auth_apr_person_fn`('$company_code','$emp_key','auth') as resps";
        $row_details = $this->EmployeeDetails->query($auth);
        $emp_ids = $row_details['0']['0']['resps'];
        //debug($auth);
        // exit;
        ...
        if ($emp_ids != '0' && $emp_ids != null) {
            $emp_condition = ' and emp_pkey != ' . $emp_key . ' and emp_pkey in (' . $emp_ids . ')';
        } else {
            $emp_condition = ' and emp_pkey != ' . $emp_key . ' ';
        }
        ...
        $arr_emp = $this->EmployeeConfig->find(
            ...
            array(
                ...
                'conditions' => array(
                    'EmployeeConfig.type' => 'LAPPR',
                    'EmployeeConfig.emp_fkey' => $emp_key,
                    'EmployeeConfig.status' => 1
                )
            )
        );
        ...
        // }
        //end
        // debug($arr_leave_details['APPROVEDBYNAME']);
        // debug($arr_leave_details['APPROVEDBY']);
        // debug($arr_leave_details['AUTHORIZEDBYNAME']);
        // debug($arr_leave_details['ISAutherizedby']);

        //            echo json_encode(array(
        //                'approvedname' => $arr_leave_details['APPROVEDBYNAME'],
        //                'approvedby' => $arr_leave_details['APPROVEDBY'],
        //                'authorisedbyname' => $arr_leave_details['AUTHORIZEDBYNAME'],
        //                'authorisedby'=>$arr_leave_details['ISAutherizedby']
        //            ));
        $response = array();

        // Check if the values are set in $arr_leave_details
        if (isset($arr_leave_details['APPROVEDBYNAME'])) {
            $response['approvedname'] = $arr_leave_details['APPROVEDBYNAME'];
        }

        if (isset($arr_leave_details['APPROVEDBY'])) {
            $response['approvedby'] = $arr_leave_details['APPROVEDBY'];
        }

        if (isset($arr_leave_details['AUTHORIZEDBYNAME'])) {
            $response['authorisedbyname'] = $arr_leave_details['AUTHORIZEDBYNAME'];
        }

        if (isset($arr_leave_details['ISAutherizedby'])) {
            $response['authorisedby'] = $arr_leave_details['ISAutherizedby'];
        }
        ...
        //debug(approvedname);
        //$this->set('arr_leave_details', $arr_leave_details);
        //$this->set('_serialize', ['arr_leave_details']);
     ...
     public function getLeaveType()
    {
        ...
        $arr_adminpriv = $this->SalaryHeadItems->query("select emp_mgt_priv from emp_proff where emp_fkey='$cur_emp_key'");
        $access= isset($arr_adminpriv['0']['emp_proff']['emp_mgt_priv'])?$arr_adminpriv['0']['emp_proff']['emp_mgt_priv']:0;
        if($company_code == 'HRBL' && $access ==1){
         $arr_leave_type = $this->SalaryHeadItems->find("all", array(
            "conditions" => array(
                ...
                . "and salary_head_item_pkey IN(select distinct salary_head_item_fkey from leavepolicy "
    ...
    public function saveLeaveEntry($leaveentryId = 0)
    {
        ...
        $cremp_key = isset($arr_form_data['Currentemp']) ? $arr_form_data['Currentemp'] : 0;
        ...
        $file_type = isset($arr_form_data['filename']) ? $arr_form_data['filename'] : '';
        $authorised = isset($arr_form_data['ISAutherizedby']) ? $arr_form_data['ISAutherizedby'] : '';
        ...
        try {
            ...
            if (($fromdate >= $att_startdate1) && ($todate <= $att_enddate1)) {
                $yearmonth = date('Y-m-d', strtotime($att_enddate1));
                $arr_attendance_register = $this->LeaveRequests->query("select count(*) cnt from attendance_register where month_year=date_format('$yearmonth','%Y-%m') and isdelete='N' and emp_fkey= '$cremp_key' ");
                $count = $arr_attendance_register['0']['0']['cnt'];
            } else if (($fromdate <= $att_enddate1) && ($todate >= $att_enddate1)) {
                $yearmonth = date('Y-m-d', strtotime($att_enddate1));
                $arr_attendance_register = $this->LeaveRequests->query("select count(*) cnt from attendance_register where month_year=date_format('$fromdate','%Y-%m') and isdelete='N' and emp_fkey= '$cremp_key' ");
                $yearmonth1 = date('Y-m-d', strtotime($fromdate . ' + 1 months'));
                $arr_attendance_register1 = $this->LeaveRequests->query("select count(*) cnt from attendance_register where month_year=date_format('$yearmonth1','%Y-%m') and isdelete='N' and emp_fkey= '$cremp_key' ");
                $count = $arr_attendance_register['0']['0']['cnt'] + $arr_attendance_register1['0']['0']['cnt'];
            } else if (($fromdate >= $att_enddate1) && ($todate >= $att_enddate1)) {
                $yearmonth = date('Y-m-d', strtotime($att_enddate1 . ' + 1 months'));
                $arr_attendance_register = $this->LeaveRequests->query("select count(*) cnt from attendance_register where month_year=date_format('$yearmonth','%Y-%m') and isdelete='N' and emp_fkey= '$cremp_key' ");
        ...
        $yearmonthdate = date('Y-m-01', strtotime($att_enddate1));
        ...
        if ($leaveentryId) {
            $emp = $this->LeaveRequests->query("select emp_fkey from leaveentries where LEAVEENTRYID = '$leaveentryId'");
            $cremp_key = $emp['0']['leaveentries']['emp_fkey'];
            $data['EMP_fkey'] = $cremp_key;
            ...
            if (isset($arr_attendance_register) && $arr_attendance_register['0']['0']['cnt'] != 0) {
                $message = 'Leave cannot be changed, Attendance verified. ';
                $resp["success"] = false;
                $resp["leaveentryId"] = 0;
                $resp['warningmessage'] = '';
                $resp["message"] = "Leave cannot be cancelled, Attendance verified. ";
                return json_encode($resp);
            }
            ...
            try {
                $this->LeaveRequests->updateAll(
                    $arr_data,
                    array('LeaveRequests.LEAVEENTRYID' => $leaveentryId)
                );
            } catch (Exception $ex) {
                $resp["success"] = false;
                $resp["leaveentryId"] = 0;
            ...
            if ($myLeaveAction == 'Cancellation Applied') {
                ...
                $data['applied_date'] = isset($arr_leave_details1['LeaveRequests']['applied_date']) ? $arr_leave_details1['LeaveRequests']['applied_date'] : '';
                $data['leave_days'] = isset($arr_leave_details1['LeaveRequests']['leave_days']) ? $arr_leave_details1['LeaveRequests']['leave_days'] : '';


                if (isset($arr_leave_details1['LeaveRequests']['APPROVED_date'])) {
                    $auth_pkey = isset($arr_leave_details1['LeaveRequests']['APPROVEDBY']) ? $arr_leave_details1['LeaveRequests']['APPROVEDBY'] : '';
                } else {
                    $auth_pkey = isset($arr_leave_details1['LeaveRequests']['ISAutherizedby']) ? $arr_leave_details1['LeaveRequests']['ISAutherizedby'] : '';
                }
            ...
            $arr_count_atte_exists = $this->LeaveRequests->query("SELECT count(*) AS COUNT FROM emp_detail_timeattandance WHERE emp_pkey = '$cremp_key' and  att_date between '$fromdate' and '$todate' and present = 'P/P' and yearmonth='$yearmonthdate'");
            $attendancecount = isset($arr_count_atte_exists[0][0]['COUNT']) ? $arr_count_atte_exists[0][0]['COUNT'] : 0;
            if ($leavestatus != 'Applied') {
                if ($attendancecount > 0) {
                    $resp["message"] = "Attendance already existing in the range of this leave. Please remove it, before applying.";
                    $resp["success"] = false;
                    return json_encode($resp);
                }
            }
            ...
            $data['ISAutherizedby'] = $authorised;
            ...
            $data['salary_head_item_fkey'] = $leavetype = $arr_form_data['salary_head_item_fkey'];
            $data['cctome'] = $cc;
            $notified_all = $this->LeaveRequests->query("select notified_by from leavepolicy left join leavepolicy_group on 
                 (leavepolicy_group.LEAVEPOLICY_GROUP_ID = leavepolicy.LEAVEPOLICY_GROUP_ID) 
                  left join emp_proff on (emp_proff.LEAVEPOLICY_GROUP_ID = leavepolicy_group.LEAVEPOLICY_GROUP_ID) where emp_fkey = $cremp_key and salary_head_item_fkey = '$leavetype'");
            $notified_all = isset($notified_all['0']['leavepolicy']['notified_by']) ? $notified_all['0']['leavepolicy']['notified_by'] : '';
            $noti_list =  isset($notified_all) ? explode(',', $notified_all) : '';
            if (!empty($noti_list)) {
                foreach ($noti_list as $val) {
                    $notify_toemp = $this->LeaveRequests->query("select email from emp_details where emp_pkey = '$val'");
                    $notifymail = isset($notify_toemp['0']['emp_details']['email']) ? $notify_toemp['0']['emp_details']['email'] : '';
                    if ($cc != '') {
                        if ($notifymail != '') {
                            $cc = $val . ',' . $cc;
                        }
            ...
            //creating cc sending employees email string
            $notifyto = isset($arr_form_data['Notifiedby']) ? $arr_form_data['Notifiedby'] : '';
            if ($notifyto > 0) {
                $notify_toemp = $this->LeaveRequests->query("select email from emp_details where emp_pkey = '$notifyto'");
                $notifymail = isset($notify_toemp['0']['emp_details']['email']) ? $notify_toemp['0']['emp_details']['email'] : '';
                if ($cc != '') {
                    if ($notifymail != '') {
                        $cc = $notifyto . ',' . $cc;
            ...
            $from_date = $arr_form_data['FROMDATE'];
            $to_date = $arr_form_data['TODATE'];
            ...
            try {
                ...
                //$arr_count_leave_exists = $this->LeaveRequests->query("select count(*) AS COUNT from emp_leave_transactions where leave_date between '$fromdate' and '$todate' and leave_session = 3 and LEAVESTATUS in ('Applied','Approved','Authorized','CancellationOfApproved','CancellationOfAuthorized') and LEAVEENTRYID in (select LEAVEENTRYID  from leaveentries where EMP_fkey = $cur_emp_key)");
                if ($arr_form_data['FROMHALF'] == 1) {
                    $arr_count_leave_exists = $this->LeaveRequests->query("select count(*) AS COUNT from emp_leave_transactions where leave_date between '$fromdate' and '$todate' and leave_session in (1,3) and Leavestatus in ('Applied','Approved','Authorized') and LEAVEENTRYID in (select LEAVEENTRYID  from leaveentries where EMP_fkey = $cremp_key)");
                } else {
                    $arr_count_leave_exists = $this->LeaveRequests->query("select count(*) AS COUNT from emp_leave_transactions where leave_date between '$fromdate' and '$todate' and leave_session in (2,3) and Leavestatus in ('Applied','Approved','Authorized') and LEAVEENTRYID in (select LEAVEENTRYID  from leaveentries where EMP_fkey = $cremp_key)");
                }
                ...
                if ($count == 0) {
                    if ($arr_form_data['FROMHALF'] == 1 && $arr_form_data['TOHALF'] == 1) {
                        $arr_count_leave_exists1 = $this->LeaveRequests->query("select count(*) AS COUNT from emp_leave_transactions where leave_date between '$fromdate' and '$todate' and leave_session = 1 and Leavestatus in ('Applied','Approved','Authorized') and LEAVEENTRYID in (select LEAVEENTRYID  from leaveentries where EMP_fkey = $cremp_key)");
                    ...
                    if ($arr_form_data['FROMHALF'] == 2 && $arr_form_data['TOHALF'] == 2) {
                        $arr_count_leave_exists1 = $this->LeaveRequests->query("select count(*) AS COUNT from emp_leave_transactions where leave_date between '$fromdate' and '$todate' and leave_session = 2 and Leavestatus in ('Applied','Approved','Authorized') and LEAVEENTRYID in (select LEAVEENTRYID  from leaveentries where EMP_fkey = $cremp_key)");
                        $count = isset($arr_count_leave_exists1[0][0]['COUNT']) ? $arr_count_leave_exists1[0][0]['COUNT'] : 0;
                    }
                }
            } catch (Exception $ex) {
            ...
            //debug($approved_pkey);
            //exit;
            if ($arr_authorized_emp['0']['emp_details']['email'] && $auth_pkey !== $approved_pkey) {


                $this->sendauthorizationmail($outputParameter1, $auth_email);
            }

            //Approved by - Edited by Akshay on 8-11-2023
            $approved_email = array('Email' => $arr_approved_emp['0']['emp_details']['email'], 'Name' => $arr_approved_emp['0']['emp_details']['first_name'], 'Appliedby' => $applied_emp['0']['emp_details']['first_name']);
            if ($arr_approved_emp['0']['emp_details']['email']) {

                $this->sendsanctiontionmail($outputParameter1, $approved_email);
                // $this->sendsanctiontionmail($outputParameter1, $approved_email);
            }

            //Sanctioned by  - Edited by Akshay on 8-11-2023
            if (count($arr_leavepolicy) > 0) {
                $sanctioned_email = array('Email' => $arr_leavepolicy['0']['emp_details']['email'], 'Name' => $arr_leavepolicy['0']['emp_details']['first_name'], 'Appliedby' => $applied_emp['0']['emp_details']['first_name']);
                if ($arr_leavepolicy['0']['emp_details']['email']) {


                    $this->sendsanctiontionmail($outputParameter1,  $sanctioned_email);
                }
            }
     ...
     * On 22 March 2015
     */
    public function criterias($pkey = 0, $fromdate = 0, $todate = 0)
    {
        $this->autoRender = FALSE;

        // check for leave on present day-------------------------//
        $arr_count_atte_exists = $this->LeaveRequests->query("SELECT count(*) AS COUNT FROM emp_detail_timeattandance WHERE emp_pkey = '$pkey' and  att_date between '$fromdate' and '$todate' and present = 'P/P' ");
        $count = isset($arr_count_atte_exists[0][0]['COUNT']) ? $arr_count_atte_exists[0][0]['COUNT'] : 0;
        if ($count > 0) {
            $resp["message"] = "Attendance already existing in the range of these Leave(s) . Please remove it, before applying.";
            $resp["success"] = false;
            return $resp;
        }
    ...
    public function grandLeave()
    {
        ...
        if ($cur_emp_key != 0 && $leaveentryId != 0) {
            ...
            if ($actionType != 'Reject' && $leave_status == 'Applied') {
                $responsedata = $this->criterias($pkey, $fromdate, $todate);
    ...
    public function GetLeaveBalance($salary_head_item_fkey = "", $emp = "", $end_date = "")
    {
        ...
        if (isset($arr_leave_policy['0']['LeavePolicy']['REMARKS']) && $arr_leave_policy['0']['LeavePolicy']['REMARKS'] == 'compoff') {
            $get_finyear = $this->LeaveRequests->query("select fin_year  from fin_year where lcase(Year_status)='open' and vattr1 = 0 and is_current_finyear='Y' and status=1
            ...
            $arr_leavelist = $this->EmployeeDetails->query("select id,break_off_date,first_half from scheduled_break_off where emp_fkey = $cur_emp_key and type = 'W' and coff_status = 'N' and status = 1 and break_off_date between '" . $finyear_start . "' and '" . $finyear_end . "'");
            // debug("select break_off_date,first_half from scheduled_break_off where emp_fkey = $cur_emp_key and type = 'W' and coff_status = 'N' and status = 1 and break_off_date between '".$from."' and '".$to_date."'");
            if ($arr_leavelist) {
        ...
        if ($company_code == 'MBCT' || $company_code == 'GEDE') {
   ...
   and branch_code= (select branch_code from emp_details where emp_Pkey='$emp_fkey' ) ORDER BY fin_year DESC LIMIT 1 "); // '$from_date' between start_month and end_month
    ...
    function sendauthorizationmail($output, $auth)
    {
        ...
        //$subject=$applied_emp.$applied_emp_id.'- Leave Request' ;
        if ($action === 'Approved') {
            // Employee Name with ID- Leave Request Approved
            $subject = $applied_emp . $applied_emp_id . ' - Leave Request Approved';
        } elseif ($action === 'Rejected') {

            $subject = $applied_emp . $applied_emp_id . ' - Leave Denied';
        } elseif ($action === 'Cancellation Approved') {
            $subject = 'Your Leave Cancellation Request has been approved successfully.';
        } elseif ($action === 'Cancellation Authorized') {

            $subject = $applied_emp . $applied_emp_id . '- Leave Cancelled';
        } elseif ($action === 'Cancellation') {
            $subject = $applied_emp . $applied_emp_id . ' -Leave Cancelled';
        } elseif ($action === 'Approval') {
            $subject = $applied_emp . $applied_emp_id . '  - Leave Request First Level Approved';
        } else {
            $subject = $applied_emp . $applied_emp_id . ' -Leave Request';
        }
        ...
        try {
            ...
            if ($addressess) {
                foreach ($addressess as $key => $value) {
                    $value = isset($value['0']['emp_details']['email']) ? $value['0']['emp_details']['email'] : '';
    ...
    //Edited by Akshay on 8-11-2023
    function sendsanctiontionmail($output, $auth)
    {
        // debug($output);
        // debug($auth);exit;
        $this->autoRender = FALSE;
        ...
        if ($action === 'Approved') {
            ...
            $msgs = 'Leave has been applied by one of your team-mates in ' . $companyname . '.';
        }

        if ($action === 'Approved') {
            $subject = $applied_emp . $applied_emp_id . '-  Leave Request Second Level Approved.';
        } elseif ($action === 'Rejected') {
            $subject = 'Sorry. Your Leave Request has been rejected.';
        } elseif ($action === 'Cancellation Approved') {
            $subject = 'Your Leave Cancellation Request has been approved successfully.';
        } elseif ($action === 'Cancellation Authorized') {
            $subject = 'Your Leave Cancellation Request has been authorized successfully.';
        } elseif ($action === 'Approval') {
            $subject = $applied_emp . $applied_emp_id . '  - Leave Request Second Level Approved';
        } else {
            $subject = $applied_emp . $applied_emp_id . ' -Leave Request';
    ...
    function sendfinalapprovalmail($output, $auth)
    {
        ...
        if ($action === 'Approved') {
            $subject =   $applied_emp . $applied_emp_id . '- Leave Request Third Level Approved.';
        } elseif ($action === 'Rejected') {
            $subject =  $applied_emp . $applied_emp_id . '- Leave Denied.';
        } elseif ($action === 'Approval') {
            $subject =  $applied_emp . $applied_emp_id . ' - Leave Request Second  Level Approved';
...
<tr>
    ...
    function callLeaveTransactionProcedure($leaveentryId = '')
    {
        if (!empty($leaveentryId)) {
            ...
            //$query = "CALL leave_transaction_prc($outputParameter,@Perror_message);";
            //  $proc_result = $this->LeaveRequests->query($query);
            // debug($proc_result);
            return $out;
        }
    }
