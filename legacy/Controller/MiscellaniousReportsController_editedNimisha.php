...
//            'LeaveBalanceSummary' => 'Leave Taken Summary Report',
    ...
    private function generatesummaryreport($mode) {
        ...
        try{
            $arr_empleaverequests = $this->LeaveRequests->query('SELECT (SELECT CONCAT(first_name," ",last_name) from emp_details where emp_pkey =  LeaveRequests.ISAutherizedby) as Authorized_name,(SELECT CONCAT(first_name," ",last_name) from emp_details where emp_pkey =  LeaveRequests.APPROVEDBY) as Approved_name,`LeaveRequests`.`LEAVEENTRYID`,`LeaveRequests`.`FROMHALF`,`LeaveRequests`.`TOHALF`,`LeaveRequests`.`leave_days`,`LeaveRequests`.`REMARKS`,Units.branch_name,EmployeeDetails.branch_code, CONCAT(first_name, " ", last_name) AS emp_name,Info.* ,`SalaryHeadItems`.`item` AS `leave_type`,`LeaveRequests`.`applied_date`, `LeaveRequests`.`FROMDATE`, `LeaveRequests`.`TODATE`, `LeaveRequests`.`LEAVESTATUS` FROM `leaveentries` AS `LeaveRequests` LEFT JOIN `salary_head_items` AS `SalaryHeadItems` ON (`LeaveRequests`.`salary_head_item_fkey` = `SalaryHeadItems`.`salary_head_item_pkey`)'
                . ' LEFT JOIN `emp_details` AS `EmployeeDetails` ON (`LeaveRequests`.`EMP_fkey` = `EmployeeDetails`.`emp_pkey`)'
                . ' LEFT JOIN `salary_head_items` AS `salary_head_items` ON (`LeaveRequests`.`salary_head_item_fkey` = `salary_head_items`.`salary_head_item_pkey`) '
                ...
                . ' where EmployeeDetails.status = 1 and ' . $str_conditions . 'ORDER BY EmployeeDetails.emp_pkey desc ');
                ...
                if ($arr_form_data['select-criteria1'] == 'SalaryHeadItems') {
                    ...
                    try{
                        $arr_empleaverequests = $this->LeaveRequests->query('SELECT ed.emp_pkey,ed.branch_code, CONCAT(first_name, " ", last_name)'
                            ...
                            . 'emp_details ed join emp_proff ep on (ed.emp_pkey=ep.emp_fkey)
                                ...
                                and lp.salary_head_item_fkey=' . $leavepolicygroupid . ' and ed.status = 1 ORDER BY ed.emp_pkey ');
                    } catch (Exception $ex) {
                    ...
                    try{
                        $arr_empleaverequests = $this->LeaveRequests->query("SELECT ed.emp_pkey,ed.branch_code, CONCAT(first_name, ' ', last_name)
                                ...
                                and ed.branch_code=ed.branch_code
                                and ed.status = 1 
                                and lp.salary_head_item_fkey=lp.salary_head_item_fkey and ed.status = 1 ORDER BY ed.emp_pkey ");
                    } catch (Exception $ex) {
