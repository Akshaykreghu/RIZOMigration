 ...
 */
class SiteAttendanceController extends AppController
{
     ...
     * @var array
     */
    public $uses = array('EmployeeDetails', 'Useraccess', 'EmployeeMenu', 'SiteMaster', 'SiteAttendance', 'SiteTransactions', 'Access_site', 'SiteHistory');
    ...
    public function form($site_pkey = 0)
    {
        ...
        $company_code = strtoupper($this->Session->read('company_code')); // Edited by Akshay on 21-8-2025
 ...
 from emp_menu EmployeeMenu  left join user_access as u on(u.menu_id = EmployeeMenu.menu_id) and u.user_fkey='$emp_pkey' and u.active='Y' where EmployeeMenu.parent_id = 0 and EmployeeMenu.active = 'Y'");
    ...
    public function lists($param = "")
    {
        ...
        if ($user_group == 2) {
            ...
            $temp_site_keys2 = $this->Useraccess->query("select distinct site_fkey from access_site left join site on(site.site_pkey = access_site.site_fkey) "
                . "left join site_transactions on(site.site_pkey = site_transactions.site_fkey) where emp_fkey=" . $cur_emp_key . " and site_fkey not in (select site_pkey from site where user_pkey=" . $cur_emp_key . " and status=1) $cond1 and site.status=1 and site_transactions.status=1");
            $site_key = array();
            foreach ($temp_site_keys1 as $val) {
                $site_key[] = $val['site']['site_pkey'];
            }
        ...
        //        $arr_useraccess = $this->Useraccess->query("select ifnull((select sum(emp_count) from site_transactions where site_fkey = site.site_pkey and status = '1' ),0) as counts ,site.* from site"
        //                . " where status = '1' $cond $branch_condition ORDER BY site.site_pkey DESC limit $limit offset $offset ");
    ...
    public function saveSite()
    {
        ...
        for ($i = 0; $i < $arr_data_count; $i++) {
            ...
            if ($arr_data['editkey'][$i] == 0) {
                ...
                $site_data['srate'] = $arr_data['TdSRate'][$i];
                 // Edited by Akshay on 21-8-2025
                    if ($company_code == 'DEMO' || $company_code == 'GLET'|| $company_code == 'ABSG'|| $company_code == 'SCRT') {
                        $site_data['eratess_28'] = $arr_data['Rate28'][$i];
                        $site_data['eratess_29'] = $arr_data['Rate29'][$i];
                        $site_data['eratess_30'] = $arr_data['Rate30'][$i];
                        $site_data['eratess_31'] = $arr_data['Rate31'][$i];
                    } else {
                ...
                // Edited by Akshay on 21-8-2025
                    if ($company_code == 'DEMO' || $company_code == 'GLET'|| $company_code == 'ABSG'|| $company_code == 'SCRT') {
                        $update_data['eratess_28'] = $arr_data['Rate28'][$i];
                        $update_data['eratess_29'] = $arr_data['Rate29'][$i];
                        $update_data['eratess_30'] = $arr_data['Rate30'][$i];
                        $update_data['eratess_31'] = $arr_data['Rate31'][$i]; // Default expense rate field
                    } else {
    ...
    public function get_shift($month = '', $fkey = '')
    {
        ...
        $site_fkey = $fkey;
        $date_att = $month;
        //        $site_data = $this->Useraccess->query("select distinct day_time_seq_fkey,site_attendance.status,day_time_desc,on_dutty1,off_dutty1,working_time1,site_transactions.start_date_effective,
