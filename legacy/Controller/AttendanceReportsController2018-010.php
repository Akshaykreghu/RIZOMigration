        ...
        if (isset($arr_leavepolicygroupids) && !empty($arr_leavepolicygroupids)) {
            foreach ($arr_leavepolicygroupids as $leavepolicygroupid) {
                ...
                if ($arr_form_data['hidden-criteria1'] == "Units") {
                    $conditions[] = 'AttendanceRegister.branch_code="' . $leavepolicygroupid . '" and AttendanceRegister.month_year = "' . $report_month . '" and EmployeeDetails.status = 1 ';
                } else {
                    $conditions[] = 'EmployeeDetails.emp_pkey="' . $leavepolicygroupid . '" and AttendanceRegister.month_year = "' . $report_month . '" and EmployeeDetails.status = 1 ';
                }
...
//                     .$str_conditions)
                ...
                if ($arr_form_data['select-criteria1'] == 'EmployeeDetails') {
                    $arr_leavepolicy_details = $this->DeviceAttendance->query("SELECT `DeviceAttendance`.LOGDATE,`DeviceAttendance`.C1,"
                            ...
                            . " WHERE DeviceAttendance.status = 'Y' and  EmployeeDetails.status = 1 and `LOGDATE` between '$from' and '$to' and EmployeeDetails.emp_pkey='$leavepolicygroupid' "
                    ...
                    $arr_leavepolicy_details = $this->DeviceAttendance->query("SELECT `DeviceAttendance`.LOGDATE,`DeviceAttendance`.C1,`DeviceAttendance`.C2,"
                            ...
                            . "LEFT JOIN `employee_info` AS `info` ON (`info`.`emp_pkey` = `EmployeeDetails`.`emp_pkey`)"
                            . "WHERE DeviceAttendance.status = 'Y' and EmployeeDetails.status = 1 and `LOGDATE` between '$from' and '$to' and DeviceAttendance.branch_code='$leavepolicygroupid'"
                            . " ORDER BY `EmployeeDetails`.`emp_id`,LOGDATE");
                }
...
//                     .$str_conditions)
                ...
                $arr_leavepolicy_details = $this->DeviceAttendance->query("SELECT `OTMASTER`.*,"
                        ...
                        . "LEFT JOIN `employee_info` AS `Info` ON (`Info`.`emp_pkey` = `EmployeeDetails`.`emp_pkey`)"
                        . "WHERE EmployeeDetails.status = 1 and OTMASTER.month = '$from' and EmployeeDetails.branch_code = '$leavepolicygroupid' and is_verified = 'Y' ");
