 ...
 */
class EmpattendanceController extends AppController {
     ...
     * @var array
     */
    public $uses = array('EmployeeDetails', 'SalaryHeadItems', 'Units', 'AttendanceRegister', 'DbConfig', 'EmployeeProfessionalDetails');
    ...
    public function showregister() {
        ...
        $br_conditions = array('EmployeeProfessionalDetails.attr1' => $emp_fkey);
        $Branch_joins = array(
            array(
                'table' => 'branches',
                'alias' => 'Branches',
                ...
                'foreignKey' => false,
                'conditions' => array(
                    'Branches.branch_code = EmployeeProfessionalDetails.emp_branch'
                )
            )
        );
        $arr_branches = $this->EmployeeProfessionalDetails->find("all", array(
            "joins" => $Branch_joins,
            "fields" => array("distinct(EmployeeProfessionalDetails.emp_branch) branch_code,Branches.branch_name branch_name"),
            "conditions" => $br_conditions,
            "order" => array("Branches.branch_name" => "ASC")
                )
        );
        $arr_branches_list = array();
        foreach ($arr_branches as $branch) {
            $arr_branches_list[] = array_merge($branch['EmployeeProfessionalDetails'], $branch['Branches']);
        }
        $this->set('arr_branches', $arr_branches_list);
        ...
        //On 01 Oct 2016
        $this->set('hierarchy',true);
        $this->render('/Attendance/showregister');
    }
    ...
    public function showregistertab($verified = 0) {
        ...
        //On 01 Oct 2016
        $this->set('hierarchy',true);
        $this->render('/Attendance/showregistertab');
    }
        ...
        foreach ($arr_register as $key => $value) {
            ...
            foreach ($value["AttendanceRegister"] as $key1 => $val) {
                if ($dayCount <= $numberOfDays && strpos($key1, "FIELD") === 0) {
                    ...
                    $int_days_present += ((isset($arr_field[0]) && ($arr_field[0] == 'P' || $arr_field[0] == 'WFH' || $arr_field[0] == 'COFF' || $arr_field[0] == 'NA')) ? ((count($arr_field) == 1) ? 1 : 0.5) : 0) + ((isset($arr_field[1]) && ($arr_field[1] == 'P' || $arr_field[1] == 'WFH' || $arr_field[1] == 'COFF' || $arr_field[1] == 'NA')) ? 0.5 : 0);
    ...
    public function updateregisterentries($registerid = 0) {
        ...
        //On 01 Oct 2016
        $this->render('/Attendance/updateregisterentries');
    }
    ...
    public function verifiedpdf($branch, $month, $emp, $mode) {
        ...
        $logged_in_emp_fkey = $this->Session->read('emp_fkey');
        
        $conditions = array(
            'AttendanceRegister.isdelete="N"',
            'EmployeeProfessionalDetails.attr1' => $logged_in_emp_fkey
        );
