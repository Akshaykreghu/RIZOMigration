...
class FieldSurveyController extends AppController {
     ...
     * @var array
     */
    public $uses = array('SurveyType','EquipmentType','Item','SiteWork', 'Site', 'Equipments', 'EfsrTickets','EmployeeDetails','PackageMaster','Item','MaterialRequest','MaterialRequestDetails');
    ...
    public function home(){
        ...
        $arr_types = $this->EquipmentType->query("SELECT * FROM equipment_type WHERE status = '1' ");
...
//        $this->set("all_store", $all_store = $this->Store->find("all", array("conditions" => array('status' => 1))));
    ...
    public function form_ticket($efsr_tickets_pkey = 0){
        ...
        //all item      
        $this->set("all_item", $all_item = $this->SiteWork->find("all", array("conditions" => array('status' => 1))));
        $arr_types = $this->SiteWork->query("SELECT efsr_tickets_pkey,survey_type.type_name,efsr_tickets.survey_type_fkey,efsr_tickets.site_fkey,efsr_equipments_master_fkey,ticket_no,survey_type_fkey,type_name,emp_fkey,emp_details.first_name,emp_approved.first_name approved_by,concat(equipments_name,'|',manufacturer) equipments_name,efsr_id,site_name,sap_site_id,site_id
...
join efsr_equipments_master on (efsr_equipments_master_fkey=efsr_equipments_master_pkey)
  where efsr_tickets.status = '1' and efsr_tickets_pkey = '$efsr_tickets_pkey' ");
    ...
    public function form($id = 0){
        ...
        $arr_site_list = $this->EquipmentType->query("SELECT efsr_site_pkey,site_name FROM efsr_site WHERE status = '1' ");
        $this->set('arr_site_list', $arr_site_list);
        $arr_equipment_data = array();
        $id = isset($_GET['id'])?$_GET['id']:null;
        if(isset($id)){
            $arr_equipment_data = $this->EquipmentType->query("SELECT * FROM efsr_equipments_master WHERE status = '1' and efsr_equipments_master_pkey = '$id' ");
        }
    ...
    public function lists() {
        ...
        $id = 1;
        $counts = $this->Equipments->query("SELECT count(*) as count FROM `efsr_equipments_master` where status = '1' $branch_condition ");

        $count = $counts[0][0]['count'];
        
        $arr_att = $this->Equipments->query("SELECT efsr_equipments_master.*,equipment_type.equipment_name from efsr_equipments_master left join equipment_type on (equipment_type.equipment_type_pkey = efsr_equipments_master.equipments_type) where efsr_equipments_master.status = '1' $branch_condition ORDER BY $sort $order limit $limit offset $ofst ");
        ...
        foreach ($arr_att as $key => $value) {
            ...
            $out['equipments_type'] = isset($value['equipment_type']['equipment_name']) ? $value['equipment_type']['equipment_name'] : '';
    ...
    public function lists_site() {
        ...
        $id = 1;
        $counts = $this->EfsrTickets->query("SELECT count(*) as count from efsr_tickets join efsr_site on (efsr_site.efsr_site_pkey = site_fkey)
 join survey_type on (type_pkey=survey_type_fkey)
 join emp_details on (emp_details.emp_pkey=emp_fkey)
  join emp_details as emp_approved on (emp_approved.emp_pkey=approved_by)
join efsr_equipments_master on (efsr_equipments_master_fkey=efsr_equipments_master_pkey)
  where efsr_tickets.status = '1' $branch_condition ");
        ...
        $arr_att = $this->EfsrTickets->query("SELECT efsr_tickets_pkey,ticket_no,survey_type_fkey,type_name,emp_fkey,emp_details.first_name,emp_approved.first_name approved_by,concat(equipments_name,'|',manufacturer) equipments_name,efsr_id,site_name,sap_site_id,site_id
 from efsr_tickets join efsr_site on (efsr_site.efsr_site_pkey = site_fkey)
 join survey_type on (type_pkey=survey_type_fkey)
 join emp_details on (emp_details.emp_pkey=emp_fkey)
  join emp_details as emp_approved on (emp_approved.emp_pkey=approved_by)
...
//         debug("SELECT efsr_tickets_pkey,ticket_no,survey_type_fkey,type_name,emp_fkey,emp_details.first_name,emp_approved.first_name approved_by,concat(equipments_name,'|',manufacturer) equipments_name,efsr_id,site_name,sap_site_id,site_id
// from efsr_tickets join efsr_site on (efsr_site.efsr_site_pkey = site_fkey)
// join survey_type on (type_pkey=survey_type_fkey)
// join emp_details on (emp_details.emp_pkey=emp_fkey)
//  join emp_details as emp_approved on (emp_approved.emp_pkey=approved_by)
    ...
    public function save() 
   {
	...
	$this->EfsrTickets->useDbConfig = $this->Session->read('ds');
        ...
        if($filterby_Equipment != 0){
        	...
        	$arr_filterby_Equipment =  Set::extract('/efsr_equipments_master/efsr_equipments_master_pkey/.', $this->EfsrTickets->query("SELECT efsr_equipments_master_pkey FROM efsr_equipments_master WHERE site_fkey = '$site_fkey' and equipments_type = '$survey_type' ") );
        }
        // print_r($arr_filterby_Equipment); die();
        foreach ($arr_filterby_Equipment as $key => $value) {
        	//$this->EfsrTickets->query("INSERT INTO efsr_tickets(site_fkey,efsr_equipments_master_fkey,survey_type_fkey,emp_fkey,approved_by) VALUES('$site_fkey','$filterby_Equipment','$survey_type','$emp_fkey','$approved_pkey') ");   
              //edited by megha 29_04_19 start
            //$arr_ticket['ticket_no']=1000;
            $arr_ticket['site_fkey']=$site_fkey;
            $arr_ticket['efsr_equipments_master_fkey']=$filterby_Equipment;
            $arr_ticket['survey_type_fkey']=$survey_type;
            $arr_ticket['emp_fkey']=$emp_fkey;
            $arr_ticket['approved_by']=$approved_pkey;
            //
            $arr_ticket_no = $this->EfsrTickets->save($arr_ticket);
            $insertedId = $this->EfsrTickets->getInsertid();
            $this->EfsrTickets->updateAll(array('ticket_no'=>'concat(efsr_tickets_pkey,"",site_fkey)'),array('efsr_tickets_pkey'=>$insertedId));
            $this->EfsrTickets->updateAll(array('efsr_id'=>'concat(site_fkey,"",ticket_no)'),array('efsr_tickets_pkey'=>$insertedId));
                 //edited by megha 29_04_19 end
                }
    ...
    public function ticket(){
        ...
        //all item  
        $this->SurveyType->useDbConfig = $this->Session->read('ds');
        $arr_types = $this->SurveyType->query("SELECT * FROM survey_type WHERE status = '1' ");
        $this->set('arr_types', $arr_types);
        $this->set("all_item", $all_item = $this->SiteWork->find("all", array("conditions" => array('status' => 1))));
    }
...
//        );
    ...
    public function jsons_equipments($site = '',$branch = '') {
        $this->autoRender = false;
        $this->Equipments->useDbConfig = $this->Session->read('ds');
          // debug($branch);
        //debug($_REQUEST['q']);
        ...
        if(isset($site) && $site != ''){
            $branch_cond = " and efsr_equipments_master.site_fkey = $site ";
        }else{
            $branch_cond = " ";
        }
        if(isset($equip) && $equip != ''){
            $site_cond = " and survey_type.type_pkey = $branch ";
        }else{
            $site_cond = " ";
        }
//        $branch_array = $this->EmployeeDetails->query("select * from emp_details left join emp_proff on (emp_details.emp_pkey = emp_proff.emp_fkey) where status = 1  $q_condition $emp_condition ORDER BY emp_pkey DESC ");
        $branch_array = $this->Equipments->query(" SELECT efsr_equipments_master.*,equipment_type.*,survey_type.type_pkey,CONCAT(equipment_name,' - ', equipments_name,' | ', survey_type.type_code,' - ',survey_type.type_name) as equi_name FROM equipment_type,survey_type,efsr_equipments_master
 WHERE equipment_type.equipment_type_pkey = survey_type.equipment_type_fkey
  and efsr_equipments_master.equipments_type=survey_type.type_pkey $site_cond $branch_cond ");
 
//       debug(" SELECT efsr_equipments_master.*,equipment_type.*,survey_type.type_pkey,CONCAT(equipment_name,' - ', equipments_name,' | ', survey_type.type_code,' - ',survey_type.type_name) as equi_name FROM equipment_type,survey_type,efsr_equipments_master
// WHERE equipment_type.equipment_type_pkey = survey_type.equipment_type_fkey
//  and efsr_equipments_master.equipments_type=survey_type.type_pkey $site_cond $branch_cond ");
       // debug($branch_array);
        //$datas = $this->request->data;
        $array = array();
        $branches = array();
        $branches[] = array(); //array("id" => "0", "text" => "ALL");
        foreach ($branch_array as $key => $value) {
            $branches[] = array(
...
//        $branch_array = $this->EmployeeDetails->query("select * from emp_details left join emp_proff on (emp_details.emp_pkey = emp_proff.emp_fkey) where status = 1  $q_condition $emp_condition ORDER BY emp_pkey DESC ");
        $branch_array = $this->SurveyType->query("SELECT * FROM survey_type WHERE status = 1  ORDER BY type_pkey DESC");
//        debug("Select emp_pkey,first_name,last_name,emp_id From emp_details join emp_proff on (emp_details.emp_pkey = emp_proff.emp_fkey) 
//                                                       where  status = 1 and emp_details.emp_pkey in (select emp_pkey from emp_site_detail_timeattandance $branch_condition)  ORDER BY emp_pkey");
//        debug($branch_array);
        //$datas = $this->request->data;
...
//                            if ($ctcuploadtype == 1) {
                                ...
                                $arr_Emp = $this->SiteWork->query("SELECT * FROM `efsr_site` WHERE `site_id`='$site_code' ");
...
//                            if ($ctcuploadtype == 1) {
                                ...
                                $arr_survey = $this->SiteWork->query("SELECT * FROM `survey_type` WHERE `type_code`='$survey_code' ");
                                
                                $technician_id = isset($row['Site Code']) ? $row['Site Code'] : '';
                                $arr_Emp = $this->UserCredentials->query("SELECT * FROM `user_credentials` WHERE `site_id`='$site_code' ");
