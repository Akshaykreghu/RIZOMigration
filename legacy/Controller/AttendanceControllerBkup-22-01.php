...
class AttendanceController extends AppController {
     ...
     * @var array
     */
    public $uses = array('EmployeeDetails', 'Units', 'AttendanceRegister', 'DbConfig','SalaryHeadItems','LeaveRequests');
...
WHERE DATE_FORMAT(month_year,'%Y-%m') = '$month' ");
    ...
    public function updateregisterentries($registerid = 0) {
        ...
        if ($registerid != 0) {
            ...
            $month = $emp_pkeys['0']['attendance_register']['month_year'].'-01';
            $years = $this->AttendanceRegister->query("select fin_year from fin_year where Year_status = 'OPEN' and vattr1 = 0 and is_current_finyear = 'Y' and status = '1' and branch_code= (select branch_code from emp_details where emp_Pkey='$emp_pkey' ) ORDER BY fin_year DESC LIMIT 1");
...
//            DEBUG($arr_leaves_heads);
        ...
        foreach($arr_leaves_heads as $key => $val){
            ...
            $lbalance = $this->AttendanceRegister->query("select leave_balance_inthe_month_fn('$emp_pkey','$slary_head_item_pkey','$month','$year') as LeaveBalance");
...
//                        debug($arr_requestdata['reg-date-' . $i]);
                        ...
                        $arr_updateentries['FIELD' . $index] = isset($arr_requestdata['reg-date-' . $i]) ? '"' . $arr_requestdata['reg-date-' . $i] . '/'.$arr_requestdata['reg-date-' . $i].'"' : '""';
                    }
...
VALUES ('$d', '$emp_fkey', '$f', '1');");
    ...
    public function AddHalfLeave($head = '',$day = '',$emp_fkey = 0,$session = ""){
        ...
        $this->LeaveRequests->useDbConfig = $this->Session->read('ds');
        $leaves = array();
        $arr_leaves = $this->AttendanceRegister->query("SELECT salary_head_item_pkey FROM `salary_head_items` WHERE `item_type` = 'Leave' AND `status` = '1' and occurance = '$head' LIMIT 50"); 
        $leaveentryId = 0;
        $leaves['salary_head_item_fkey'] = isset($arr_leaves['0']['salary_head_items']['salary_head_item_pkey'])?$arr_leaves['0']['salary_head_items']['salary_head_item_pkey']:0;
        $leaves['applied_date'] = $day;
        $leaves['LEAVESTATUS'] = "Approved";
