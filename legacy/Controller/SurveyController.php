...
class SurveyController extends AppController {
    ...
    public function surveyTypesList($asArray = false){
        ...
        $result = $this->SurveyType->query("select type_pkey,type_code ,type_name,CONCAT(equipment_code, ' | ',equipment_name) as equipment_name
                 from survey_type as st LEFT JOIN equipment_type as et on (et.equipment_type_pkey = st.equipment_type_fkey and et.status = 1) 
                 where st.status = 1 order by type_pkey  desc".$pageData['limit'].$pageData['offset']);
        $count = $this->SurveyType->query("select count(type_pkey) as count from survey_type as st 
                    LEFT JOIN equipment_type as et on (et.equipment_type_pkey = st.equipment_type_fkey and et.status = 1) 
                    where st.status = 1 ");         
        ...
        echo json_encode(array("total"=>$total_count,"rows"=>$survey_types));
    }
    ...
    public function getSurveyCategory($asArray = false,$fields ='category_pkey,type_fkey as survey_type, category_code,category_name,category_order'){
        ...
        $result = $this->SurveyCategory->query('select '.$fields.' from survey_category where status = 1 '.$survey_type_cond." ".$pageData['limit'].$pageData['offset']  );
        $count  = $this->SurveyCategory->query('select count(category_pkey)as count from survey_category where status = 1' .$survey_type_cond);
        if ($asArray == true)
            return $result;
        $survey_categories = array();
        foreach ($result as $key => $val) {
             $survey_categories[] =  array_merge($val['survey_category'],isset($val[0])?$val[0]:array());
        }
        
        $total_count = (isset($count[0]) && isset($count[0][0]) && isset($count[0][0]['count']))?$count[0][0]['count']:0;
        echo json_encode(array("total"=>$total_count,"rows"=>$survey_categories));
    }
    ...
    public function deleteSurveyCategory(){
        ...
        echo json_encode(array('msg' => 'Survey Head deleted successfully.'));
    }
    /*** Options ***/
    public function options(){
        ...
        $fields = " category_pkey, concat (category_code,' - ',category_name) category_code   ";
    ...
    public function getOptionsList($asArray = false,$fields=''){
        ...
        $query = 'select '.$fields. ',GROUP_CONCAT(oiv.option_items_values order by oiv.option_values_order ASC) as o_values 
                        ...
                        WHERE  oi.status= 1  '.$cond.' group by oi.`option_items_pkey` '.$pageData['limit'].$pageData['offset']."";  
        
        $countQuery = "SELECT count(option_items_pkey)as count FROM `option_items` as oi WHERE `status` = 1 ".$cond."";

        $result = $this->OptionItems->query($query);
        $count = $this->OptionItems->query($countQuery);
        ...
        $total_count = (isset($count[0]) && isset($count[0][0]) && isset($count[0][0]['count']))?$count[0][0]['count']:0;
        echo json_encode(array("total"=>$total_count,"rows"=>$option_items));
    ...
    public function getTickets($status = 0){
        ...
        $result = $this->EfsrTickets->query("select efsr_tickets_pkey, ticket_no, survey_type_fkey, concat(type_code,' ',type_name)type_name, emp_fkey,
                                    ...
                                    where ".$cond." order by 1 desc ".$pageData['limit'].$pageData['offset']."");     

        $count = $this->EfsrTickets->query("select count(efsr_tickets_pkey) as count
                                    FROM efsr_tickets join efsr_site on (efsr_site.efsr_site_pkey = site_fkey ".$site_cond.")
                                    JOIN survey_type on (type_pkey=survey_type_fkey)
                                    JOIN emp_details on (emp_details.emp_pkey=emp_fkey ".$technican_cond." )
                                    JOIN emp_details as emp_approved on (emp_approved.emp_pkey=approved_by)
        ...
        $total_count = (isset($count[0]) && isset($count[0][0]) && isset($count[0][0]['count']))?$count[0][0]['count']:0;
        echo json_encode(array("total"=>$total_count,"rows"=>$tickets));
    }
    ...
    public function getLastServiceHtml($efsr_equipments_master_fkey = 0 , $ticket_id = 0){
...
//                where efsr_tickets.status = '1' and efsr_equipments_master_fkey= ".$efsr_equipments_master_fkey." order by service_date desc limit 4");
        //edited by megha on 13_06_19 report view 
        $lastServices = $this->EfsrTickets->query("SELECT ticket_no as service_no,emp_details.first_name as technician,
                emp_approved.first_name approved_by,efsr_tickets.creation_date as service_date
                from efsr_tickets join efsr_site on (efsr_site.efsr_site_pkey = site_fkey)
                join survey_type on (type_pkey=survey_type_fkey)
                ...
                join contacts on (contact_id_fkey=contact_id)
                join efsr_equipments_master on (efsr_equipments_master_fkey=efsr_equipments_master_pkey)
                where efsr_tickets.status in (1,2) and efsr_equipments_master_fkey= ".$efsr_equipments_master_fkey." and ticket_no = ".$ticket_id." order by service_date desc limit 4");
...
//edited by megha on 13_06_19 report view
        ...
        $lastServicesHtml = $this->getLastServiceHtml($efsr_equipments_master_fkey,$ticket_id);
