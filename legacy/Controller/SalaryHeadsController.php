 ...
 $this->SalaryHeadItems->useDbConfig = $this->Session->read('ds');
           ...
           $salaryHeadItemsDB = $this->SalaryHeadItems->find("all",array("conditions" => array("SalaryHeadItems.status" => 1, "SalaryHeadItems.head_fkey" => $id)));
            ...
            foreach ($salaryHeadItemsDB as $shikey => $shivalue) {
                ...
                if ($shivalue['SalaryHeadItems']['head_fkey'] == $id) {
                    ...
                    $salaryHeads[$key]['part'] = ($shivalue['SalaryHeadItems']['item_part']== 'Indirect')?'Admin Only':$shivalue['SalaryHeadItems']['item_part'];
   ...
   $this->SalaryHeads->useDbConfig = $this->Session->read('ds');
    ...
    public function addsalaryheads() { //echo "hlo";die();
        ...
        if($this->checksalaryheadexists($arr_form_data['SALARYHEAD_NAME'],$temp) > 0){
                   return json_encode(array("success"=>false,"msg"=>"Salary Head Exists ")); 
                } 
    }
...
public function checksalaryheadexists($codecount = '',$SALARYHEAD_ID=0){
        $this->autoRender = false;
        
        $arr_requestdata = $this->request->data;
        
        $SALARYHEAD_NAME = isset($arr_requestdata['SALARYHEAD_NAME'])?$arr_requestdata['SALARYHEAD_NAME']:$codecount;
        
        $int_typecount = 0;
        if($SALARYHEAD_NAME != ''){
            $this->SalaryHeads->useDbConfig = $this->Session->read('ds');
            $int_typecount = $this->SalaryHeads->find("count",array(
                    'conditions' => array('SalaryHeads.head_desc' => $SALARYHEAD_NAME,'SalaryHeads.status' => 1,"SalaryHeads.head_pkey !='$SALARYHEAD_ID'")
                )
            );
        }
   ...
   public function checksalaryheaditemexists($SALARYHEADITEM_ID=0){
        $this->autoRender = false;
        
        $arr_requestdata = $this->request->data;
        
        $ITEM_NAME = isset($arr_requestdata['ITEM_NAME'])?$arr_requestdata['ITEM_NAME']:'';
        
        $int_typecount = 0;
        if($ITEM_NAME != ''){
            $this->SalaryHeadItems->useDbConfig = $this->Session->read('ds');
            $int_typecount = $this->SalaryHeadItems->find("count",array(
                    'conditions' => array('SalaryHeadItems.item' => $ITEM_NAME,'SalaryHeadItems.status' => 1, 'SalaryHeadItems.head_fkey '=> $SALARYHEADITEM_ID)
                )
            );
        }
        echo $int_typecount;
    }
     public function checkshortnameexists($short_name = 0){
        $this->autoRender = false;
    
        $arr_requestdata = $this->request->data;
        // $pkey = $arr_form_data['expense_type_pkey'];
        $short_name   = isset($arr_requestdata['short_name']) ? $arr_requestdata['short_name'] : '';
    
        $int_shortnamecount = 0;
        if ($short_name  != '') {
            $this->SalaryHeadItems->useDbConfig = $this->Session->read('ds');
            $int_shortnamecount = $this->SalaryHeadItems->find("count", array(
            'conditions' => array(
            'SalaryHeadItems.occurance' => $short_name,
            'SalaryHeadItems.status' => 1,
            'NOT' => array('SalaryHeadItems.occurance' => 'LOP')
              )
           ));
        }
        echo $int_shortnamecount;
    }
