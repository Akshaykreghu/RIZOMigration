...
class EditPunchesController extends AppController
{
    ...
    // 			$restrictedCompanies = [
    //     'ABSG','VGFS','VSFS','DRRC','DJIC','AGNG','AYRK','SHRD',
    //     'INFR','ALDS','CKWR','BNGL','DVDS','STPN','TSSM',
    //     'ELKT','IMSC','GTRA','FRSG','VGNN'
    // 		];
    ...
    public function index($emp_id = '')
    {
        ...
        if (!$feature_fkey)
            $feature_fkey = 39; // Use 39 for Edit Punches by default

        $arr_branches = array(); // Initialize to prevent warning
        ...
        if ($user_group == 2 && !in_array($company_code, $not_allowed_companies)) {
            ...
            if (!empty($accessResults)) {
                foreach ($accessResults as $row) {
                    ...
                    if (strtoupper($r['is_hierarchy']) == 'Y') {
                        $is_hierarchy = 'Y';
                    }
                }
            }
            ...
            if ($is_hierarchy == 'Y') {
                ...
                if (!empty($branches)) {
                    foreach ($branches as $b) {
                        $arr_branches[] = isset($b['branches']) ? $b['branches'] : (isset($b[0]) ? $b[0] : (isset($b['Units']) ? $b['Units'] : $b));
                    }
                }
            ...
            // 3. Initial Employee List (Filtered by Hierarchy/Allocated)
            if ($is_hierarchy == 'Y') {

                // Build branch condition safely
                $branchCondition = "";
                $params = array($user_fkey, $user_fkey);

                if (!empty($allocated_branches)) {
                    $branch_list = "'" . implode("','", $allocated_branches) . "'";
                    $branchCondition = " AND ep.emp_branch IN ($branch_list)";
                }

                // Try hierarchy + branch
    ...
    ", $params);
                ...
                // 🔥 FALLBACK → if no result, ignore branch filter
                if (empty($results)) {
                    $results = $this->EmployeeDetails->query("
            SELECT ed.emp_id 
            FROM emp_details ed
        ...
        FROM emp_details ed
        JOIN emp_proff ep ON ep.emp_fkey = ed.emp_pkey
        WHERE ep.emp_branch IN ($branch_list)
          AND ed.emp_pkey != ?
          AND ed.status = 1
        ORDER BY ed.first_name ASC
    ", array($user_fkey));
            ...
            $results = $this->EmployeeDetails->query("SELECT emp_id FROM emp_details WHERE status = 1 ORDER BY first_name ASC");
        }

        $allowed_emp_ids = array();
        foreach ($results as $row) {
            $allowed_emp_ids[] = isset($row['ed']['emp_id']) ? $row['ed']['emp_id'] : (isset($row['emp_details']['emp_id']) ? $row['emp_details']['emp_id'] : (isset($row[0]['emp_id']) ? $row[0]['emp_id'] : ''));
        }

        // Filter original API list
        $arr_employees = array_filter($arr_employees, function ($employee) use ($allowed_emp_ids) {
            return in_array($employee->id, $allowed_emp_ids);
        });
        $arr_employees = array_values($arr_employees);
        ...
        $data = $this->CentralUserCredentials->find('first', array(
            'conditions' => array(
                'CentralUserCredentials.company_code' => $this->Session->read('company_code')
            ),
            'fields' => array('CentralUserCredentials.plan_id'),
            'recursive' => -1
        ));

        $planId = !empty($data) ? (int) $data['CentralUserCredentials']['plan_id'] : null;
        $this->set('planId', $planId);
        $this->set('user_group', $user_group);
    }
    ...
    //         ");
    //             $sc = isset($shift_config[0]['working_day_time_procedures']) ? $shift_config[0]['working_day_time_procedures'] : array();
    //             $is_exception = isset($sc['is_exception']) ? $sc['is_exception'] : '';

    //             if ($is_exception == 1) {
    //                 // Fetch Shift Exceptions - edited by antigravity
    ...
    //                 $is_wo = ($is_exception == 1 && isset($sc[$day_name]) && $sc[$day_name] == 'N');
    //                 $is_half_wo = ($is_exception == 1 && isset($sc[$day_name . '_F']) && $sc[$day_name . '_F'] == 'Y');
    ...
    //             // Fetch finalized attendance register data - edited by antigravity
    ...
    //             WHERE leaves.EMP_fkey = '$emp_pkey' AND elt.Leavestatus IN ('Approved','Authorized')
    ...
    //                     if (!empty($reg_status)) {
    //                         $reg_status = str_ireplace('LOP', 'LOP', $reg_status);
    //                     }
    //                 }

    //                 // Unified Status Parts - edited by antigravity
    //                 $status_parts = ['', ''];
    //                 $l_statuses = [];
    //                 $day_special_status = ''; // Track if the day is HO or WO

    //                 // 1. Check Holiday/Week Off
    //                 if (isset($holiday_map[$att_date])) {
    //                     $status_parts = ['HO', 'HO'];
    //                     $day_special_status = 'HO';
    //                 } else {
    //                     $day_name = date('l', strtotime($att_date));
    //                     $day_num = date('d', strtotime($att_date));
    //                     $week_index = ceil($day_num / 7);

    //                     $exception_wo = ($is_exception == 1 && isset($exception_map[$day_name][$week_index])) ? $exception_map[$day_name][$week_index] : null;

    //                     if ($exception_wo === 'Y') {
    //                         $status_parts = ['WO', 'WO'];
    //                         $day_special_status = 'WO';
    //                     } elseif ($exception_wo === 'N') {
    //                         $status_parts = ['A', 'A'];
    //                     } else {
    //                         if (isset($sc[$day_name]) && $sc[$day_name] == 'N') {
    //                             $status_parts = ['WO', 'WO'];
    //                             $day_special_status = 'WO';
    //                         } elseif (isset($sc[$day_name . '_F']) && $sc[$day_name . '_F'] == 'Y') {
    //                             $status_parts = ['', 'WO'];
    //                             $day_special_status = '/WO';
    //                         }
    //                     }
    ...
    //                     // 3. Merge Attendance (P)
    //                     $present_str = trim($val[$base_table]['present']);
    //                     $present_parts = explode('/', $present_str);
    //                     if (isset($present_parts[0])) {
    //                         $p0 = strtoupper(trim($present_parts[0]));
    //                         if ($p0 == 'P' || ($p0 == 'A' && !empty($val[$base_table]['att_in_time']))) {
    //                             $status_parts[0] = $p0;
    //                         }
    //                     }
    //                     if (isset($present_parts[1])) {
    //                         $p1 = strtoupper(trim($present_parts[1]));
    //                         if ($p1 == 'P' || ($p1 == 'A' && !empty($val[$base_table]['att_in_time']))) {
    //                             $status_parts[1] = $p1;
    //                         }
    //                     }

    //                     // 2. Check Leaves for the day - edited by antigravity
    //                     if (isset($leave_map[$att_date])) {
    //                         foreach ($leave_map[$att_date] as $l_data) {
    //                             if (!empty($l_data['elt']['leave_session']) && !empty($l_data['shi']['occurance'])) {
    //                                 $session = $l_data['elt']['leave_session'];
    //                                 $occ = $l_data['shi']['occurance'];
    //                                 $l_status = isset($l_data['elt']['Leavestatus']) ? $l_data['elt']['Leavestatus'] : '';

    //                                 if ($session == 3) {
    //                                     $status_parts = [$occ, $occ];
    //                                 } elseif ($session == 1) {
    //                                     $status_parts[0] = $occ;
    //                                 } elseif ($session == 2) {
    //                                     $status_parts[1] = $occ;
    ...
    //                     if ($status_parts[0] == '' && $status_parts[1] == '') {
    //                         $final_present_str = '';
    //                     } else {
    //                         $final_present_str = ($status_parts[0] == $status_parts[1] && in_array($status_parts[0], ['WO', 'HO'])) ? $status_parts[0] : implode('/', $status_parts);
    //                     }

    //                     // Append day status if not already fully represented (e.g. P/P WO)
    //                     if (!empty($day_special_status) && $final_present_str != $day_special_status && strpos($final_present_str, $day_special_status) === false) {
    //                         $final_present_str .= ' ' . $day_special_status;
    //                     }
    //                     $approval_str = !empty($l_statuses) ? ' (' . implode(', ', array_unique($l_statuses)) . ')' : '';
    ...
    //                         'status' => !empty($reg_status) ? $reg_status : (trim($final_present_str . $approval_str) == 'A/A' && empty($val[$base_table]['att_in_time']) ? '' : trim($final_present_str . $approval_str)),
    ...
    //                             (strpos($final_present_str, 'A') !== false ? 'red' : (strpos($final_present_str, 'P/P') !== false ? 'green' : 'black')),
    //                         'editable' => $iseditable,
    //                     ];
    ...
    //                 } else {
    //                     // Empty day record - edited by antigravity
    ...
    //                     // Append day status if not already fully represented
    //                     if (!empty($day_special_status) && $final_present_str != $day_special_status && strpos($final_present_str, $day_special_status) === false) {
    //                         $final_present_str .= ' ' . $day_special_status;
    //                     }
    //                     $approval_str = !empty($l_statuses) ? ' (' . implode(', ', array_unique($l_statuses)) . ')' : '';
    ...
    //                         'att_out_time' => '',
    //                         'duration' => '',
    //                         'status' => !empty($reg_status) ? $reg_status : (trim($final_present_str . $approval_str) == 'A/A' ? '' : trim($final_present_str . $approval_str)),
    //                         'status_color' => !empty($reg_status) ?
    //                             (strtoupper($reg_status) == 'P/P' ? 'green' :
    //                                 (in_array(strtoupper($reg_status), ['A/A', 'LOP/LOP', 'ABSENT']) ? 'red' : 'black')) : (strpos($final_present_str, 'A') !== false ? 'red' : (strpos($final_present_str, 'P/P') !== false ? 'green' : 'black')),
    //                         'editable' => $iseditable,
    //                         'shift_name' => ''
    //                     ];
    //                 }
    ...
    //                 // Unified Status Merging - edited by antigravity
    //                 $p = trim($val[$base_table]['present']);
    //                 $h = trim($val[$base_table]['holiday']);
    //                 $l = trim($val[$base_table]['leaves']);
    //                 $w = trim($val[$base_table]['weekoff']);
    //                 $o = trim($val[$base_table]['others']);

    //                 $get_part = function ($str, $index) {
    //                     $parts = explode('/', $str);
    //                     if (count($parts) > 1)
    ...
    //                 $final_parts = ['A', 'A'];
    //                 foreach ([0, 1] as $i) {
    //                     foreach ([$l, $p, $h, $w, $o] as $s) {
    //                         $val_part = $get_part($s, $i);
    //                         if (!empty($val_part) && strtoupper($val_part) != 'A' && strtoupper($val_part) != '') {
    //                             $final_parts[$i] = $val_part;
    //                             break;
    //                         }
    //                     }
    //                 }
    //                 if ($final_parts[0] == '' && $final_parts[1] == '') {
    //                     $status = '';
    //                 } else {
    //                     $status = ($final_parts[0] == $final_parts[1] && in_array($final_parts[0], ['WO', 'HO'])) ? $final_parts[0] : implode('/', $final_parts);
    //                 }

    //                 // Append nature of day for fallback branch
    //                 $day_nature = '';
    //                 if (!empty($h) && strtoupper($h) != 'A')
    //                     $day_nature = 'HO';
    //                 elseif (!empty($w) && strtoupper($w) != 'A')
    //                     $day_nature = 'WO';

    //                 if (!empty($day_nature) && $status != $day_nature && strpos($status, $day_nature) === false) {
    //                     $status .= ' ' . $day_nature;
    //                 }
    //                 $status = ($status == 'A/A' && empty($val[$base_table]['att_in_time'])) ? '' : $status;
    //                 $arr_output['status'] = $status;
    //                 $status_color = (strpos($status, 'A') !== false) ? 'red' : ((strpos($status, 'P/P') !== false) ? 'green' : 'black');
    //                 // debug($arr_output);
    ...
    public function listpunches()
    {
        ...
        ");
            ...
            // Edited by Akshay on 25-5-2026
            $ot_data = $this->EmployeeDetails->query("
        SELECT att_date, isdelete
        FROM emp_ot_timeattandance
        WHERE emp_pkey = '$emp_pkey'
        AND att_date BETWEEN '$attendance_start' AND '$attendance_end'
        ");

            $ot_map = [];

            foreach ($ot_data as $ot) {
                $date = $ot['emp_ot_timeattandance']['att_date'];
                $isdelete = $ot['emp_ot_timeattandance']['isdelete'];

                // Default enabled
                if (!isset($ot_map[$date])) {
                    $ot_map[$date] = 1;
                }

                // If any record is N → disable
                if ($isdelete === 'N') {
                    $ot_map[$date] = 0;
                }
            }
        ...
        ");
            ...
            // Generate grid rows
            foreach ($period as $index => $att_date) {
                $view_button = isset($ot_map[$att_date]) ? $ot_map[$att_date] : 1;
                // Edited by Akshay on 3-12-2025
                // debug($iseditable);
                ...
                if (isset($attendance_map[$att_date])) {
                    ...
                    if (isset($leave_map[$att_date])) {
                        foreach ($leave_map[$att_date] as $l_data) {
                            if (!empty($l_data['elt']['leave_session']) && !empty($l_data['shi']['occurance'])) {
                                ...
                                $clean_occ = trim(str_ireplace(['Approved', 'Authorized', 'Applied', 'Rejected'], '', $occ));
                                if ($session == 3) {
                                    $status_parts = [$clean_occ, $clean_occ];
                                } elseif ($session == 1) {
                                    $status_parts[0] = $clean_occ;
                                } elseif ($session == 2) {
                                    $status_parts[1] = $clean_occ;
                                }

                                if (!empty($l_status)) {
                                    //edited by athira on 04-05-2026
                                    $reg_parts = array_map('trim', explode('/', $reg_status));
                                    if (empty($reg_status) || in_array(trim($occ), $reg_parts)) {
                                        $l_statuses[] = $l_status;
                                    }
                                    //ended by athira on 04-05-2026
                                }
                            }
                        }
                    }
                    ...
                    $arr_output = [
                        ...
                        'ad_remarks' => $val[$base_table]['ad_remarks'],
                        //edited by athira on 04-05-2026
                        'status' => !empty($reg_status) ? trim($reg_status . $approval_str) : (trim($final_present_str . $approval_str) == 'A/A' && empty($val[$base_table]['att_in_time']) ? '' : trim($final_present_str . $approval_str)),
                        //ended by athira on 04-05-2026
                        'shift_name' => $shift_name,
                        'joining_date' => $val['emp']['joining_date'],
                        'status_color' => !empty($reg_status) ?
                            (strtoupper($reg_status) == 'P/P' ? 'green' : (in_array(strtoupper($reg_status), ['A/A', 'LOP/LOP', 'ABSENT']) ? 'red' : 'black')) : (strpos($final_present_str, 'A') !== false ? 'red' : (strpos($final_present_str, 'P/P') !== false ? 'green' : 'black')),
                        'editable' => $iseditable,
		...
		");
            ...
            // Fetch leave details separately - edited by antigravity
            $leaves_query = $this->EmployeeDetails->query("
                SELECT 
                    elt.leave_date, 
                    elt.leave_session, 
                ...
                AND elt.leave_date BETWEEN '" . date('Y-m-01', strtotime($yearmonth)) . "' AND '" . date('Y-m-t', strtotime($yearmonth)) . "'
            ");

            $leave_map = [];
            foreach ($leaves_query as $lq) {
                $status_tag = $lq['elt']['Leavestatus'];
                if (isset($leave_map[$lq['elt']['leave_date']])) {
                    $leave_map[$lq['elt']['leave_date']] .= '/' . $status_tag;
                } else {
                    $leave_map[$lq['elt']['leave_date']] = $status_tag;
                }
            }
           ...
           $status_color = "black";
                ...
                $p = trim($val[$base_table]['present']);
                $h = trim($val[$base_table]['holiday']);
                $l = trim(str_ireplace(['Approved', 'Authorized', 'Applied', 'Rejected'], '', $val[$base_table]['leaves']));
                $w = trim($val[$base_table]['weekoff']);
                $o = trim($val[$base_table]['others']);

                $att_date = $val[$base_table]['att_date'];
                $leave_status = isset($leave_map[$att_date]) ? $leave_map[$att_date] : '';

                $reg_status = '';
                if ($att_reg_final_record) {
                    $day_num = (int)date('d', strtotime($att_date));
                    $field_name = 'FIELD' . $day_num;
                    $reg_status = isset($att_reg_final_record[$field_name]) ? $att_reg_final_record[$field_name] : '';
                    if (!empty($reg_status)) {
                        $reg_status = trim(str_ireplace(['Approved', 'Authorized', 'Applied', 'Rejected'], '', $reg_status));
                    }
                ...
                // Enrichment for finalized status - edited by antigravity
                if (!empty($reg_status)) {
                    $status = trim($reg_status . (!empty($leave_status) ? ' (' . implode('/', array_unique(array_map('trim', explode('/', $leave_status)))) . ')' : ''));
                } elseif (!empty($leave_status)) {
                    $status = trim($status . ' (' . implode('/', array_unique(array_map('trim', explode('/', $leave_status)))) . ')');
                }

                $arr_output['status'] = $status;
                $arr_output['leaves'] = !empty($leave_status) ? implode('/', array_unique(array_map('trim', explode('/', $leave_status)))) : $l;
                $status_color = (strpos($status, 'A') !== false) ? 'red' : ((strpos($status, 'P/P') !== false) ? 'green' : 'black');
                // debug($arr_output);
                $arr_output['status_color'] = $status_color;
                //}else{
