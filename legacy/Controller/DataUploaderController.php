 ...
 */
class DataUploaderController extends AppController {
     ...
     * @var array
     */
    public $uses = array('CentralControl','EmployeeConfig', 'Family','passport','NoticePeriod', 'qualifcations', 'Promotion', 'history', 'EmployeeTaxTransactions', 'EmpTaxSalTrans', 'FinancialYear', 'UserCredentials', 'EmployeeDetails', 'Designation', 'EmployeeProfessionalDetails', 'Departments', 'Grades', 'Verticals', 'Units', 'TaxHead', 'EmployeeCTC');
    ...
    public function updateesi($site_pkey = 0) {
        $this->autoRender = false;
        //$site_pkey=[];
        //  debug($site_pkey);
        
        $this->EmployeeDetails->useDbConfig = $this->Session->read('ds');
        $arr_Emp = $this->EmployeeDetails->query("select * from update_esi ");
        
        // debug($arr_Emp);
        foreach ($arr_Emp as $value) {
            $esi = $value['update_esi']['esi'];
            $pf = $value['update_esi']['pf'];
            $uan = $value['update_esi']['uan'];
            $emp_id = $value['update_esi']['emp_id'];
            $arr_Emp_updates = $this->EmployeeDetails->query("update emp_details set esi = '$esi' , company_pf = '$pf', pf = '$uan' where emp_pkey in (select emp_fkey from emp_proff where emp_company_id = '$emp_id' )");
            //  debug($str_stage_options_html);
        }
//        echo $str_stage_options_html;
    }
...
//        debug($arr_empdetails);
    ...
    public function downloadempdataformat() {
        ...
        $emp_details_schema = $empcsvdata->getFieldHeadings('EmployeeDataUpdater'); 
...
//        $objWorkSheet->setCellValue('B3', 'Second Half = 2');
    ...
    public function downloadempctcformat($ctcuploadtype = 0, $branch = '', $employee = '') {
        ...
        if ($ctcuploadtype == 1) {
            ...
            $worksheet->setCellValueByColumnAndRow(2, 1, "ESI");
            $worksheet->setCellValueByColumnAndRow(3, 1, "PF");
            $worksheet->setCellValueByColumnAndRow(4, 1, "UAN NUMBER");
            $worksheet->setCellValueByColumnAndRow(5, 1, "ACCOUNT NUMBER");
            $worksheet->setCellValueByColumnAndRow(6, 1, "BANK");
            $worksheet->setCellValueByColumnAndRow(7, 1, "IFSC");
            $worksheet->setCellValueByColumnAndRow(8, 1, "Address");
            $worksheet->setCellValueByColumnAndRow(9, 1, "City");
            $worksheet->setCellValueByColumnAndRow(10, 1, "Zip");
            $worksheet->setCellValueByColumnAndRow(11, 1, "State");
            $worksheet->setCellValueByColumnAndRow(12, 1, "Mobile");
            $worksheet->setCellValueByColumnAndRow(13, 1, "Branches");
            $worksheet->setCellValueByColumnAndRow(14, 1, "email");
            $worksheet->setCellValueByColumnAndRow(15, 1, "Branchss");
            $worksheet->setCellValueByColumnAndRow(16, 1, "Branch");
        ...
        $this->EmployeeDetails->useDbConfig = $this->Session->read('ds');
        if ($ctcuploadtype == 2) {
            $arr_empdetails = $this->EmployeeDetails->query(" SELECT ep.emp_company_id,UserCredentials.user_id, EmployeeInfo.EmpName,ect.emp_anual_ctc "
...
//                        $arr_empctc_fields = $empcsvdata->getFieldNames('EmployeeCTC'); 
                        foreach ($arrayempdata as $key => $row) {
                            ...
                            if ($ctcuploadtype == 1) {
                                ...
                                $UaN = isset($row['UAN NUMBER']) ? $row['UAN NUMBER'] : '';
...
//                            if($UaN != '')
                            $arr_empctc_data['pf'] = $UaN; //UAN 
                            if($Esi != '')
                            $arr_empctc_data['esi'] = $Esi;
...
//                            try {
                                debug($arr_empctc_data);
                                // die();
                                $result1 = $this->EmployeeDetails->save($arr_empctc_data);
