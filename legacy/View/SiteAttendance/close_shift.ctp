<?php if (!empty($siteatt_data)) { 
 //   if ($company_code == 'ABSG' || $company_code == 'absg' ) {
  ?>
    <div id="shift_close">
...
if ($user_group == 1 || ($user_group == 2 && ($company_code == 'DEMO' || $company_code == 'demo' || $company_code == 'VGFS' || $company_code == 'vgfs' || $company_code == 'VSFS' || $company_code == 'vsfs'))) {
    if ($shift_open == 0) { ?>
        <div id="shift_open">
...
<script>
    ...
    function open_shift() {
        if (confirm("All data entered will be removed.Do you want to open the closed shift? ")) {
            ...
            var att_date = $('#filterby_date').val();
            $.ajax({
                url: livesite + 'SiteAttendance/shift_open/',
                method: 'POST',
                data: {
                    site_fkey: site_fkey,
                    day_time_seq_fkey: day_time_seq_fkey,
