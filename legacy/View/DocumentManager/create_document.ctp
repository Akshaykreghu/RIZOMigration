...
<script>
    ...
    // Edited by Akshay on 13-1-2024
    function printPreview(obj) {
        if ($('#document_form #template').val() !== '') {
            var data = {
                emp_id: $('#document_form #employee').val() || '',
                temp_id: $('#document_form #template').val() || '',
                company_id: $('#document_form #company').val() || '',
                branch_id: $('#document_form #branches').val() || '',
                supplier_id: $('#document_form #suppliers').val() || '',
                customer_id: $('#document_form #customers').val() || '',
                others_id: $('#document_form #others').val() || '',
                placeholders: placeholders,
                template: CK_TEMPLATE // CKEDITOR.instances['document_editor'].getData()
            };

            $.ajax({
                url: `${livesite}DocumentManager/printPreview/` + obj,
                type: 'POST',
                data: data,
                success: function(response) {
                    ...
                    // Write the response HTML to the iframe
                ...
                error: function(xhr, status, error) {
                    ...
                    alert('An error occurred while fetching the print preview.');
                }
            });
        } else {
            alert('Please select a template before generating the print preview.');
        }
    }
    // End
        ...
        $('#documents_table').datagrid({
            ...
            columns: [
                [
                    ...
                    <?php  if($companycode == 'absg' || $companycode == 'demo'){ ?>
                    {
                        field: 'download_link',
