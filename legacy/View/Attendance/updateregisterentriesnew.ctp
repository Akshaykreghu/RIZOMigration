...
<!--                                  <select onFocus=(this.olds=this.value) class="bg-color" onchange="leavabalance(this,this.olds);this.olds=this.value" id="reg-date-<?php echo $i; ?>" name="reg-date-<?php echo $i; ?>" class="form-control">   -->
                        ...
                        <!-- <label style="text-align:left;" class="col-md-6 control-label" for="reg-date-<?php echo $i; ?>">
    <?php echo date('d-m-Y D', strtotime($date)); ?> <?php echo $half; ?>
</label> -->

<select 
    onfocus="this.olds=this.value" 
    onchange="leavabalance(this,this.olds);this.olds=this.value" 
    id="reg-date-<?php echo $i; ?>" 
    name="reg-date-<?php echo $i; ?>" 
    class="form-control" 
    data-emp="<?php echo (string)$emp_pkey; ?>" 
    data-date="<?php echo date('Y-m-d', strtotime($date)); ?>"> <!-- ✅ use Y-m-d format -->
    
    <option value="">--No Change--</option>
    <option value="LOP">Set As LOP</option>
    <option value="WFH">Work From Home</option>
    <?php foreach($arr_leaves as $val){ ?>
        <option value="<?php echo $val['salary_head_items']['occurance']; ?>">
...
function leavabalance(s, d) {
   ...
   var emp_pkey = String($(s).data('emp'));
    var type = $(s).val(); // new leave type
    var input_string = $(s).attr('id');
    var leaveDate = $(s).data('date');
    var dates = input_string.replace("reg-date-", "");
    var half_type = $('#hid-reg-field-half-' + dates).val();
    var session = (half_type !== '' ? 0.5 : 1);

    // Save old balances
    var oldBal = {};
    if ($('#' + d).length) oldBal[d] = parseFloat($('#' + d).val() || 0);
    if ($('#' + type).length) oldBal[type] = parseFloat($('#' + type).val() || 0);

    // ❌ Frontend check: insufficient balance
    if (type && oldBal[type] < session) {
        alert("This Employee has insufficient leave balance for " + type + " leave");
        $(s).val(d || ''); // revert dropdown
        return false;
    }

    // 🔹 Send to controller
    $.ajax({
        url: "<?php echo $this->Html->url(['controller' => 'Attendance', 'action' => 'updateLeave']); ?>",
        type: "POST",
        data: {
            emp_pkey: emp_pkey,
            occurrence: type,
        ...
        success: function(response) {
            ...
            // ✅ Update balances only if everything is OK
            if (d && $('#' + d).length) {
                var bal = oldBal[d] + session;
                $('#' + d).val(bal);
                $('#' + d + "bal").html(bal);
            }

            if (type && $('#' + type).length) {
                var bal = oldBal[type] - session;
                $('#' + type).val(bal);
                $('#' + type + "bal").html(bal);
            }
        ...
        error: function() {
            alert("Error while updating leave balance in backend");
            $(s).val(d || '');
        }
    });
}
