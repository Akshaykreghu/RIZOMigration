...
<section class="content-header">
    <h1 style="text-align:left; font-size: 3em;"> Survey Options Master   </h1>
</section>
                        ...
                        <div class="row">
                            <div class="form-group">
                                <div class="col-sm-4">
                                    ...
                                    <div class="col-md-7">
                                        <select id="survey_type_filter" name="survey_type_filter" class="form-control js-example-basic-single" onchange="filter();" >
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <div class="col-md-7">
                                        <select id="survey_category_filter" name="survey_category_filter" class="form-control js-example-basic-single" onchange="filterOptions();" >
                                            <option value="">All</option>
                                            <?php foreach ($survey_categories as $key => $value) { 
                                            ?>                              
                                                <option  value="<?php echo $value['survey_category']['category_pkey']; ?>">
                                                    <?php echo $value[0]['category_code']; ?>
                                                </option>
                                            <?php 
                                                } 
                                            ?>
...
<script>
        ...
        $.ajax({
         url:'Survey/getSurveyCategoryBySurveyType',
           ...
           success: function (resp){
                ...
                var rows  = data.rows;
                for (i in rows){
                   surveyCategoryHtml += "<option value='"+rows[i].category_pkey+"'>"+rows[i].category_code+"</option>" ;
                }
    ...
    function filterCategory(survey_type){
        $.ajax({
         url:'Survey/getSurveyCategoryBySurveyType',
           type: 'POST',
           data: {
           ...
           success: function (resp){
                var surveyCategoryHtml = '<option value="0">All </option>';
                var data = $.parseJSON(resp);
                var rows  = data.rows;
                for (i in rows){
                   surveyCategoryHtml += "<option value='"+rows[i].category_pkey+"'>"+rows[i].category_code+"</option>" ;
                }
                $('#options_index_form #survey_category_filter').html(surveyCategoryHtml);
           }
       }); 
    }
    ...
    function validateOptionForm(){
        ...
        if (option_items_valObj.val() == "" && $("[name='option_items_values[]']").length == 0 
            &&  (option_item_typeObj.val() == 'Check_box' || option_item_typeObj.val() ==  'Radio_button' 
            || option_item_typeObj.val() ==  'List_box')){
