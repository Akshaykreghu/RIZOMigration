<?php if ($mode == '') { ?>
    ...
    <div class="modal-body" style="overflow-y: auto;width:100%;overflow-x:auto;">
        <h2 align="center"><b><?php echo "Employee Information" ?></b> </h2>
        ...
        <?php $hasData = array_filter($arr_employee_report_details_merged, function ($row) {
            return count(array_filter(array_diff_key($row, ['attr4' => '']))) > 0;
        });
        if (empty($arr_employee_report_details_merged) || empty($hasData)) { ?>
            ...
            <table class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <?php
                        // debug($arr_employee_report_details_merged); exit;
...
//debug($heading_arr);
//debug($arr_emp_field_headings);exit; 
                        //edited by sinsiya 06-06-2024 to view the grade in report
                        foreach ($heading_arr as $key => $val) {
                            if (trim($key) != 'locomotive' && trim($key) != 'hearing' && trim($key) != 'visual' && trim($key) != 'category_name' && trim($key) != 'category_code'  && trim($key) != 'grade_code' && trim($key) != 'contract_start_date' && trim($key) != 'contract_end_date') {
                                echo '<th style="min-width:200px;">' . $arr_emp_field_headings[$key] . '</th>';
                            }
                        }
                        ...
                        <tr>
                            ...
                            <?php foreach ($value as $key => $val) { ?>
                                ...
                                <?php } else if ($key == 'joining_date') { ?>
                                    <td><?php echo date("d-m-Y", strtotime($val)); ?></td>
                                <?php } else if (trim($key) == 'locomotive' || trim($key) == 'hearing' || trim($key) == 'visual') {
                                    if (trim($val) == 'Y') {
                                ...
                                ?>
                                    ...
                                    <?php } else if (trim($key) == 'physical_handicap') {
                                    if (trim($val) == 'Y') { ?>
                                        <td><?php echo $physical_handicap; ?></td>
                                    <?php } else { ?>
                                        <td></td>
                                    <?php }
                                    ?>
                                    <?php } else if ((trim($key) == 'category_name' || trim($key) == 'category_code')) {
                                    ...
                                    <?php } else { ?>
                                        <td></td>
                                    <?php } //edited by sinsiya 06-06-2024 to view the grade in report
                               ...
                               // }  
                                    ...
                                    if (trim($val) == 'N' && (trim($key) == 'eps' || trim($key) == 'international_worker')) { ?>
                                        <td><?php echo 'No'; ?></td>
                                    <?php } else if (trim($val) == 'Y' && (trim($key) == 'eps' || trim($key) == 'international_worker')) { ?>
                                        <td><?php echo 'Yes'; ?></td>
                                    <?php } else {
