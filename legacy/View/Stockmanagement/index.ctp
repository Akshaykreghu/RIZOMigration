...
<div class="payroll-view">
  <section class="content-header heading">
      <h1 class="text-primary-18">Stock Management</h1>
  </section>
...
$(document).ready(function () {
  ...
  const pageKey = "Stockmanagement"; // unique key for this page
  const storageKey = `last_clicked_feature_${pageKey}`;

  // ===== Fetch Attendance Features =====
  $.getJSON("Stockmanagement/getAttendanceFeatures", function (res) {
    if (res.error) {
  ...
  $(document).on("click", ".payroll-main-grid", function (e) {
    ...
    let enabled = parseInt($(this).data("enabled")); // convert to number
