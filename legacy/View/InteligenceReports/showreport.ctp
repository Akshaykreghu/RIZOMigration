...
<script>
 ...
 $('#date').hide();
  ...
  //debug(v);
       if(v == 'SummaryPayroll' || v =='Salaryslip' || v =='Salaryslipnew' || v =='Payrollslip' || v == 'GrosssalaryNew' || v == 'Grosssalary' || v=='GrosssalarySummary' || v=='BankTranfer' || v=='GrossPeriod'  || v=='BankTranferNew' || v == 'MonthlyCTCReport' || v =='LOPREPORT' || v == 'SalaryCombined' || v == 'Analysis' || v == 'InteligenceNonCompli')
{
      $('#date').show();
}
else
...
<form class="form-horizontal" method="post" action="" id="form-showreport">
    ...
    <div class="form-group" id="div-criteria1">
        ...
        <div class="col-md-2">
            <select id="select-criteria1" name="select-criteria1" style="width: 170px;" class="form-control" onchange="loadCriteriaItems(1);" >
...
<?php } ?>
    ...
    <div class="form-group">
        ...
        <?php if(trim($type != 'BankTranferNew')){?>
            ...
            <?php if($type != "Grosssalary" && $type != "Analysis" && $type != "GrosssalaryNew" && $type != "SalaryCombined" && $type != "GrossPeriod" && $type != "SummaryPayroll" && $type != "LOPREPORT" && $type != "MonthlyCTCReport"){?>
        ...
        <?php }?>
            ...
            if(trim($type) == 'BankTranferNew'){?>
        <button type="button" onclick="downloadReport('csv');" id="btn-submit3" class="btn btn-danger"><li >.csv</li></button>
           <?php }?>
        </div>
...
function downloadReport(mode){
	...
	if(criteria){
        $('#form-showreport').attr('action',livesite+'inteligenceReports/generatereport/'+type+'/'+mode);
...
<?php if($type == 'employee'){ ?>
    ...
    allempfields.init();    
    allempfields.clearAndLoad("<?php echo $this->webroot; ?>inteligenceReports/listemployeefields", "json");
...
jQuery(document).ready(function() {
 ...
 function viewReport() {
    ...
    //        return false;
            ...
            var url = livesite + 'inteligenceReports/generatereport/' + type;
...
function loadCriteriaItems(index){
    ...
    $('#div-items-criteria'+index).load(livesite+'inteligenceReports/loadcriteriaitems/'+index+'/'+criteria);
}
function addOneReportCriteria(index){
    ...
    $('#div-criteria'+newIndex).load(livesite+'inteligenceReports/addreportcriteria/'+type+'/'+newIndex+'/'+currentcriteriaschosen);
