    ...
    <div class="modal-content">
        ...
        <div class="modal-body" style="padding:15px 8px;">
                        ...
                        <div class="col-md-12">
                            ...
                            <?php if ($att_date != "" && $emp_id != "") { ?>
                                <div id="tb" style="padding:5px 5px 8px 5px; display:flex; justify-content:space-between; align-items:center; gap:10px; flex-wrap:wrap;">
                                    <!-- Left: New button -->
                                    <?php if (!$empmode == 0 && $shownewbutton == 'yes') { ?>
                                        <button
                                            id="btn-new"
                                            type="button"
                                            onclick="insert()"
            ...
            ">
                                ...
                                <table id="editpunchesbydate" class="table table-bordered table-hover" style="overflow:auto;"></table>
                            <?php } else { ?>
...
<?php if ($att_date != "" && $emp_id != "") { ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#editpunchesbydate').datagrid({
                ...
                width: 550,
                //height: '600px',
                url: livesite + "EditPunches/listpunchesbydate",
                pagination: true,
                rownumbers: true,
                singleSelect: true,
                iconCls: 'icon-edit',
                ...
                onLoadSuccess: function(data) {
                    // $('#dailyovertimediv').datagrid('load', {
                    //     month: att_date.split('-').reverse().join('-'),
                    //     monthChosen: $('#filterby_month').val() + '-01',
                    //     branch: $("#filterby_branch").val(),
            ...
            var currentPage = pager.pagination('options').pageNumber;
            var currentPageSize = pager.pagination('options').pageSize;
            ...
            // Update query params
            $('#dailyovertimediv').datagrid('options').queryParams = {
                branch: $("#filterby_branch").val(),
                monthChosen: $('#filterby_month').val() + '-01',
                month: $('#date1').val(),
                empid: $('#employee_select').val()
            ...
            pager.pagination('select', currentPage);
            // End

            $('#editpunchesbydate').datagrid('reload');
        });
    ...
    `);
        ...
        function updateShiftDate(device_attandance_seq, newDate, empid, old_shiftdate) {
            ...
            $.ajax({
                type: "POST",
                url: livesite + "EditPunches/updateShiftDate",
                data: {
        ...
        function saverow(target, row, value) {
            ...
            $.ajax({
                type: "POST",
                url: livesite + "EditPunches/savepunch/" + empid,
                data: {
                    C1: row.C1,
        ...
        function insert() {
            ...
            if (emp) {
                $('#empid').val(emp);
                //Add form
                showSmallModalForm(livesite + 'DailyOvertimeVerifyNew/form/' + emp + '/' + att_date + '/' + site_trans_fkey);
            } else {
