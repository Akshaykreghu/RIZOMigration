...
<section class="content-header">
    <h1 style="text-align:left; font-size: 3em;"> Out Pass Generator </h1>
</section>
...
<script>
    ...
    jQuery(document).ready(function() {
        if (true) {
            $('#documents_manager').datagrid({
                url: livesite + "OutPass/getDocumentsFromDatabase",
                pagination: true,
                singleSelect: true,
                rownumbers: true,
                ...
                toolbar: [{
                        ...
                        iconCls: 'icon-add',
                        handler: function() {
                            showModalForm(livesite + 'OutPass/uploadForm')
...

                    {
                        ...
                        handler: function() {
                            ...
                            if (row) {
                                if (row.status == 1) {
                                    var out_pass_pkey = row.out_pass_pkey;
                                    showLargeModalForm(livesite + 'OutPass/savePreview/' + out_pass_pkey + '/' + 1 + '/' + true);
                                } else {
                                    alert(row.type + " pass already issued.");
                                }
...

                    {
                        ...
                        handler: function() {
                            ...
                            if(rows){
                            if (rows.status !== '2') {
                                var document_id = rows.out_pass_pkey;

                                if (confirm("Are you sure want to delete ")) {
                                    $.ajax({
                                        url: livesite + "OutPass/deleteFromGrid/" + document_id,
                                        ...
                                        success: function(response) {
                                            ...
                                            if (response.message) {
                                                ...
                                                if (response.status === 'success') {
                                                    $.notify(rows.type+response.message, {
                                                        type: 'success',
                                                        allow_dismiss: true
                                                    });
                                ...
                                alert(rows.type+" out pass issued and cannot be deleted");   
                            }
                            }else {
            ...
            success: function(response) {
                ...
                if (responseObject && responseObject.status === 'success') {
                    if (true) {
                        ...
                        var overlay = $('<div class="overlay"></div>'); // Add an overlay to prevent mouse clicks
                        var toolbarOverlay = $('<div class="toolbar-overlay"></div>');
                        var overlay2 = $('<div class="overlay2"></div>'); // Add an overlay to prevent mouse clicks
                        ...
                        //modal.append(toolbarOverlay);
                        //     modal.append(overlay);
                        //     modal.append(overlay2);
                        // }
                        ...
                        // Styling for the overlay to prevent mouse clicks
                        overlay.css({
                            'position': 'absolute',
                            'top': '0',
                            'left': '7%',
                            ...
                            'z-index': '10000', // Higher z-index to cover the file
                            'overflow': 'auto'
                        });
    ...
    function downloadFile(fileKey, fileName) {
        var url = '/DocumentManager/documentdownload'; // Modify this according to your server endpoint
    ...
    function printOutPassPDF(pkey = 0, pass = false, type = '', status = 1) {
        ...
        if (status == '2') {
            if (true) {
                ...
                alert('Personal out pass cannot be downloaded more than one time.');
            }
        } else {
            if (type == 'Official') {
                alert('Officical out pass not issued.');
            } else {
                alert('Personal out pass not issued.');
            }
        }
