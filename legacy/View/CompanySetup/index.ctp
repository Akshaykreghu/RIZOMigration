...
<div class="payroll-view">
  <section class="content-header heading">
      <h1 class="text-primary-18"> Company</h1>
  </section>
...
$(document).ready(function () {
  ...
  // ===== Fetch Payroll Features =====
  $.getJSON("CompanySetup/getCompanyFeatures", function (res) {
    if (res.error) {
   ...
   let lastIndex = localStorage.getItem("last_clicked_feature");

    if (lastIndex !== null) {
        let lastClicked = $(`.payroll-main-grid[data-index="${lastIndex}"]`);

        if (lastClicked.length && !lastClicked.hasClass("disabled")) {
...
$(document).on("click", ".payroll-main-grid", function () {
    ...
    localStorage.setItem("last_clicked_feature", index);
});
