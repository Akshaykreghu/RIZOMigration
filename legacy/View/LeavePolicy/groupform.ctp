...
<script type="text/javascript">
        ...
        $('#LEAVEPOLICY_GROUP_NAME').on('change',function(){
        checkIfLeavepolicyExists();
    })
    
    function checkIfLeavepolicyExists(callback){        
        var LEAVEPOLICY_GROUP_NAME = $('#LEAVEPOLICY_GROUP_NAME').val();
        var LEAVEPOLICY_GROUP_ID = $('#LEAVEPOLICY_GROUP_ID').val();
        $.ajax({
            url: 'LeavePolicy/checkleavepolicyexists/'+LEAVEPOLICY_GROUP_ID,
            type: 'POST',
            data: {
                LEAVEPOLICY_GROUP_NAME: LEAVEPOLICY_GROUP_NAME
            },
            success: function (resp)
            {
                if(resp > 0){      
                    alert("Leave Policy Group Already Exists!!");
                    $('#LEAVEPOLICY_GROUP_NAME').val('');
                }else{
