    ...
    <script>
        ...
        jQuery(document).ready(function() {
            ...
            if (type == 'OT_MONTHWISE') {
                ...
                $('#reportsto').on('change', function() {
                    var selectedFromDate = $('#reportstartfrom').val();
                    var selectedToDate = $(this).val();

                    var fromYear = selectedFromDate.substring(0, 4);
                    var fromMonth = selectedFromDate.substring(5, 7);

                    var toYear = selectedToDate.substring(0, 4);
                    var toMonth = selectedToDate.substring(5, 7);

                    if ((toYear < fromYear) || (toYear == fromYear && toMonth < fromMonth)) {
                        alert('The "To" month cannot be earlier than the "From" month');
                        $(this).val(selectedFromDate); // Set "To" date to match "From" date
                    }else{
                        var fromDate = new Date(selectedFromDate + "-01");
                        var toDate = new Date(selectedToDate + "-01");
                        var monthsDifference = (toDate.getFullYear() - fromDate.getFullYear()) * 12 + (toDate.getMonth() - fromDate.getMonth());
                        console.log('monthsDifference', monthsDifference);
                        if (monthsDifference > 11){
                            alert('The difference between "From" and "To" dates cannot be more than 12 months');
                            $(this).val(selectedFromDate); // Set "To" date to match "From" date
                        }
        ...
        function printovertimeContent(content) {
            ...
            //Edited by Akshay on 14-8-2024
            if (type === "Overtime_Synthiet" || type == "OT_MONTHWISE") {
                printWindow.document.write('@page { size: A4 landscape; margin: 2mm; zoom: 0.9; }'); // Set to landscape mode
            } else {
