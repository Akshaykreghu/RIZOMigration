...
<table class="table no-margin" id="expense">
    <thead>
        <tr>
...
foreach ($expenseList as $row): ?>
    <tr>
     ...
     <td><?= h( $row['Branch']['branch_name']); ?></td>
       <td><?= h( $row['EmployeeExpenses']['expense_type']); ?></td>
         <td><?= h( $row['EmployeeExpenses']['expenses_amount']); ?></td>
          <td><?= h(date('d-m-Y', strtotime($row['EmployeeExpenses']['expense_date']))); ?></td>
          <td><span class="label label-success"><?= h(strtoupper($row['EmployeeExpenses']['expense_status'])); ?></span></td>
