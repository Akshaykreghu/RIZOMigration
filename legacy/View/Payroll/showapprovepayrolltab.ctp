                    ...
                    <tr>
                        ...
                        ?>
                          ...
                          //  }
                            ...
                            <!-- End -->
                            <?php
                            if ($company_code == 'DEMO' || $company_code == 'SHYD' || $company_code == 'KWMT' || $company_code == 'GLET' || $company_code == 'GAAR') {
                            ?>
                                <td>
                        ...
                        <!-- Edited by Akshay on 6-8-2025 -->
                        <td style="padding-left:10px; color: red;">
                            * Employees shown in red letters are separated
                        </td>
                        <!-- End -->
                    </tr>
                </tbody>
    ...
    <script>
        ...
        $('#approvepayrolltable').datagrid({
            ...
            rowStyler: function(index, row) {
                if (row.resigned === true) { // Edited by Akshay on 5-8-2025
                    return 'color:red;';
                }
            },
            // End
...
<?php } else { ?>
    <script>
        ...
        jQuery(document).ready(function() {
            ...
            if (companyCode === 'SHYD' || companyCode === 'DEMO' || companyCode === 'GAAR') {
...
<script>
    ...
    function sendSlipEmailtoPersons() {
        ...
        var month = $('#approvepayrollfilter #filterby_month').val();
        var companyCode = <?php echo json_encode($company_code); ?>; // Edited by Akshay on 15-7-2025
        var arrEmpWoEmail = [];

        for (var register in checkedRows) {
            // Edited by Akshay on 15-7-2025
            if (companyCode !== 'GLET' || checkedRows[register]['email'] !== '') {
                arr_emp_pkeys.push(checkedRows[register]['emp_fkey']);
                arr_payroll_pkeys.push(checkedRows[register]['payroll_master_pkey']);
            } else
            if (checkedRows[register]['email'] === '') {
                arrEmpWoEmail.push(checkedRows[register]['emp_name']);
            }
            // End
        }

        // Edited by Akshay on 15-7-2025
        if (arr_payroll_pkeys.length === 0 && arrEmpWoEmail.length > 0) {
            $.notify("Cannot proceed. Missing email(s) for " + arrEmpWoEmail.join(', '), {
                type: 'danger',
                z_index: 9999,
                delay: 5000
            });
        ...
        if (arr_payroll_pkeys.length > 0 && arrEmpWoEmail.length > 0) {
            $.notify("Warning: Some employees don't have email and will be skipped for " + arrEmpWoEmail.join(', '), {
                type: 'danger',
                z_index: 9999,
                delay: 5000
            });
        ...
        if (row) {
            ...
            // Edited by Akshay on 11-7-2025
            if (companyCode == 'DEMO' || companyCode == 'SHYD' || companyCode == 'GLET' || $company_code == 'GAAR') {
                var url_path = "SalaryReports/sendSliptoMailSBL";
            }
            // End
            $.ajax({
