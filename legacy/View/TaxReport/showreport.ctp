...
<form class="form-horizontal" method="post" action="" id="form-showreport">
    ...
    <div class="col-md-5">
     ...
     -->
        ...
        <div class="col-md-1"><b>Criteria 1</b></div>
        <div class="col-md-3">
...
function downloadReport(mode){
	...
	if(criteria){
        $('#form-showreport').attr('action',livesite+'TaxReport/generatereport/'+type+'/'+mode);
...
<?php if($type == 'employee'){ ?>
    ...
    allempfields.init();    
    allempfields.clearAndLoad("<?php echo $this->webroot; ?>TaxReport/listemployeefields", "json");
...
function viewReport(){ 
        ...
        var url = livesite+'TaxReport/generatereport/'+type;
...
function loadCriteriaItems(index){
    ...
    $('#div-items-criteria'+index).load(livesite+'TaxReport/loadcriteriaitems/'+index+'/'+criteria);
}
function addOneReportCriteria(index){
    ...
    $('#div-criteria'+newIndex).load(livesite+'TaxReport/addreportcriteria/'+type+'/'+newIndex+'/'+currentcriteriaschosen);
