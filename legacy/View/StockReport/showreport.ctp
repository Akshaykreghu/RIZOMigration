...
// $('#date').hide();
  ...
  var v = $('#filterby_reporttype').val();
       if(v == 'SummaryPayroll' || v =='Salaryslip' || v == 'Grosssalary' || v=='GrosssalarySummary' || v=='BankTranfer')
{
      $('#date').show();
}
else
...
<form class="form-horizontal" method="post" action="" id="form-showreport">
    ...
    <div class="form-group" id="div-criteria1">
        <div id="date">
        <?php if ($type == "Allocation" || $type == "StockDetail" || $type == "Material" || $type == "PO" || $type == "GRN" || $type == "StockTransfer" || $type == "PoReturn" || $type == "AllocationEMI" ) {  ?>
...
<!--            //edited by megha 16_04_2019-->
            <?php }elseif ($type == "Stock" ){ ?>
            <div class="col-md-1"><b>As on&nbsp; &nbsp;&nbsp;:</b></div>
            <div class="col-md-2">
                <div class="col-md-12" style="padding-left: 4px; padding-right: 4px; ">
                        <div class="col-md-6" style="padding-left: 4px; padding-right: 4px; ">
                            <input type="text" id="reporstfrom" name="reportfrom" class="form-control pickerDate" value="<?php echo date("Y-m-d"); ?>" />
   ...
   <!--            //edited by megha 16_04_2019-->         
      ...
      <div class="col-md-3  " id="items-criteria1" >
...
function downloadReport(mode){
	...
	if(criteria){
        $('#form-showreport').attr('action',livesite+'StockReport/generatereport/'+type+'/'+mode);
 ...
 //load item names from selected store
    function  loadItems() {
        ...
        var type = $('#hidden-report-type').val();
	var criteria = $('#select-criteria1').val();
        var url = livesite+'StockReport/itemlist/'+type;
        //var criteria = $('#select-criteria1').val();
        //alert(criteria);
          $("#items-criteria").select2(
                {
                    ...
                    allowClear: true,
                    ajax: {
                        url: livesite + "StockReport/itemlist/" ,
                        dataType: 'json',
                        delay: 250,
...
<?php if($type == 'employee'){ ?>
    ...
    allempfields.init();    
    allempfields.clearAndLoad("<?php echo $this->webroot; ?>StockReport/listemployeefields", "json");
...
function viewReport(){ 
	...
	if(criteria){
        ...
        var url = livesite+'StockReport/generatereport/'+type;
...
function loadCriteriaItems(index){
    ...
    var type = $('#hidden-report-type').val();
    $('#div-items-criteria'+index).load(livesite+'StockReport/loadcriteriaitems/'+index+'/'+criteria);
    if(type == "StockAllDetails"){
     $('#items-criteria').show();
    }
...
function addOneReportCriteria(index){
    ...
    $('#div-criteria'+newIndex).load(livesite+'StockReport/addreportcriteria/'+type+'/'+newIndex+'/'+currentcriteriaschosen);
