...
<form class="form-horizontal" method="post" action="" id="form-showreport">
    ...
    <div class="col-md-5">
     ...
     <div class="col-md-1"><b>  Month &nbsp;:</b></div>
        <div class="col-md-2">
              <select id="reportfrom" name="reportfrom" class="form-control" style="padding-left: 0px;">
...
function loadCriteriaItems(index){
    ...
    $('#div-items-criteria'+index).load(livesite+'EditedReports/loadcriteriaitems/'+index+'/'+criteria);
}
function downloadReport(mode){
	var data = $('#hidden-criterias-count').val();					   
    ...
    //        return false;
            if(itemselect){
                ...
                alert("Choose criteria items for continue download");
                return false;
            }	
	if(criteria){
        $('#form-showreport').attr('action',livesite+'EditedReports/generatereport/'+type+'/'+mode);
...
<?php if($type == 'employee'){ ?>
    ...
    allempfields.init();    
    allempfields.clearAndLoad("<?php echo $this->webroot; ?>EditedReports/listemployeefields", "json");
...
jQuery(document).ready(function() {
 ...
 function viewReport() {
    ...
    //        return false;
            ...
            var url = livesite + 'EditedReports/generatereport/' + type;
...
function addOneReportCriteria(index){
    ...
    $('#div-criteria'+newIndex).load(livesite+'EditedReports/addreportcriteria/'+type+'/'+newIndex+'/'+currentcriteriaschosen);
