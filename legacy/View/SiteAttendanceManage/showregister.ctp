...
<section style="margin-bottom: -24px;" class="content-header">
    <div class="box box-header">
        <h1 style="float:left;" class="text-primary-18">Site Attendance Register for <label id="current_register_month"><?php echo date('M-Y'); ?></h1>
<!--        <button onclick="updateamendmence();" title="If your datas not being proccessed, Please click this button to update all the
             entries gain" id="updateamendance" class="btn btn-primary pull-right pull-up" style="margin:20px;">Update Amendments</button>-->
    </div>
</section>
<!-- Main content -->
...
var arr_other_regentries = ['COFF','WFH','NA','OTHERS'];
    ...
    function updateRegisterEntries(registerid) {
     ...
     function removeAttendanceEntry() {
        ...
        if (checkedRows && checkedRows.length > 0) {
            $.ajax({
                url: livesite + controllerName + "/removeAttendanceEntry",
                type: 'post',
                data: {
                    branch: branch,
                    month: month,
    ...
    function updateamendmence()
    {
       ...
       $.ajax({
            ...
            success: function (resp) {
                $('#updateamendance').html('Update Amendmence again').attr("disabled",false);
