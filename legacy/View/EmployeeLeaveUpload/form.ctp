...
<script>
    ...
    $('#leaveuploadtable').on('submit', function(event) {
        ...
        var empPkey = $('#emp_fkey1').val();
        var leaveType = $('#leave_type').val();
        var startDate = $('#leave_start_date').val();
        var endDate = $('#leave_end_date').val();
        var startSession = $('#leave_start_session').val();
        var endSession = $('#leave_end_session').val();

        $.ajax({
            url: livesite + 'EmployeeLeaveUpload/checkLeaveDays',
            type: 'POST', // Request type (POST/GET)
            data: {
                emp_pkey: empPkey, // Data to be sent to the controller
                    ...
                    <div class="form-group">
                        <div class="col-md-12">
                            ...
                            <div class="col-sm-7">
                                ...
                                <input type="text" required="required" class="form-control strict-field disable_first" placeholder="Leave Start Date " value="<?php echo $data['leave_start_date'] ?>  " name="leave_start_date" id="leave_start_date" autocomplete="off">
                                <!-- End -->
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="col-sm-7">
                                ...
                                <select class="form-control input-md strict-field disable_first" id="leave_start_session" onchange="session_validation()" name="leave_start_session" required="required" " > 
                                    ...
                                    <option value=" 1" <?php // echo($data['leave_end_session'] == '1') ? 'selected="selected"' : ''; 
                                                        ?>>First Half</option>
                                    <option value="2" <?php // echo($data['leave_end_session'] == '2') ? 'selected="selected"' : ''; 
                                                        ?>>Second Half</option>
                    ...
                    <div class="form-group">
                        <div class="col-md-12">
                            ...
                            <div class="col-sm-7">
                                ...
                                <!-- Edited by Akshay on 9-8-2024 -->
                                <input type="text" required="required" class="form-control strict-field disable_first" placeholder="Leave end Date " value="<?php echo $data['leave_end_date'] ?>  " name="leave_end_date" id="leave_end_date" autocomplete="off">
                                <!-- End -->
                            </div>
                        </div>
                    </div>
    ...
    function session_validation() {
        ...
        if (start == end_date) {
            if ($('#leave_end_session').val() == '1' && $('#leave_start_session').val() == '2') {
                ...
                Swal.fire({
                    icon: 'warning',
                    title: 'Please choose proper leave sessions ',
                    confirmButtonText: 'OK'
                });
                //End
...
//success time hide                     
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (leaveTypes == '') {
                        ...
                        Swal.fire({
                            icon: 'warning',
                            title: 'Please Allocate Leave Policy to this Employee',
                            confirmButtonText: 'OK'
                        });
                        //End
                    }
        ...
        $('#leave_start_date').change(function() {
            ...
            var end_date = $('#leave_end_date').val();
            var emp_id = $('#emp_fkey1').val();
            if ((start) && (start.trim() != '') && (end_date) && (end_date.trim() != '')) {
                var start_date_time = new Date(start.split('-').reverse().join('-'));
                var end_date_time = new Date(end_date.split('-').reverse().join('-'));

                if (start_date_time > end_date_time) {
                    // alert("Start date should be less than end date"); //Edited by Akshay on 9-8-2024
                    //Edited by Akshay on 14-8-2024
                    Swal.fire({
                        icon: 'warning',
        ...
        $('#leave_end_date').change(function() {
            ...
            if ((start) && (start.trim() != '') && (end_date) && (end_date.trim() != '')) {
                ...
                if (start_date_time > end_date_time) {
                    // alert("End date should be greater than start date"); //Edited by Akshay on 9-8-2024
                    //Edited by Akshay on 14-8-2024
                    Swal.fire({
                        icon: 'warning',
        ...
        $('#leave_type').change(function() {
            ...
            if (emp_pkey == '') {
                ...
                Swal.fire({
                    icon: 'warning',
                    title: 'Choose an employee to apply leave',
                    confirmButtonText: 'OK'
                });
                //End
            }
            ...
            success: function(resp) {
                ...
                if (yearly_balance != 365) {
                    ...
                    if (yearly_balance <= 0) {
                        ...
                        Swal.fire({
                            icon: 'warning',
                            title: 'Employee have no leave balance to take',
                            confirmButtonText: 'OK'
                        });
                        //End
