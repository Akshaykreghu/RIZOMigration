...
<section>
    <div class="container" style="margin-bottom : 46px; margin-top : 1px;">
        <form  id="contactInfoForm" action="<?php echo $this->webroot; ?>DbConfig/salary_policy" method="post">
            <h1 style="text-align: center;"> Manage Leaves & Leave Policies </h1>
            <p style="margin-top: 17px;    text-align: center; ">Create Leave Policies as per your company rules . You can enable/disable leave types by clicking on any leave types .  </p>
            <div class="col-md-12" >
                <?php foreach($leave_heads as $val){ ?>
                <button type="button" class="btn <?php if($val['salary_head_items']['value'] == 'Y'){ ?>btn-success<?php } else { ?>btn-default<?php } ?>" style="    margin: 8px; " onclick="togglee_item(this,<?php echo $val['salary_head_items']['salary_head_item_pkey']; ?>);" ><?= $val['salary_head_items']['item']; ?></button>
                <?php } ?>
                <button class="btn btn-default" type="button" onclick="$('#loader').load(livesite+ 'LeavePolicy/index/'); "><li class="fa fa-refresh"></li></button>
            </div>
            ...
            <button type="button" id="cinfocancel" class="btn btn-info pull-right "  onclick="canceledit();" style="margin-right:5px">Skip</button>
...
<script type="text/javascript">
    function togglee_item(s,id){
        
        if($(s).attr("class") == 'btn btn-success'){
            
            $.ajax({
            url: livesite + 'DbConfig/save_leave_head'  ,
            method:'POST',
            data:{id:id,type:1},
            success: function (resp) {
                ...
                if(response.success == 0){
                    ...
                    $.notify("Leave Head Removed",{
                                        type: 'success',
                                        allow_dismiss: false
                                    });
                    $(s).addClass("btn-default").removeClass('btn-success');
                    $('#loader').load(livesite+ 'LeavePolicy/index/');
                }
            ...
            success: function (resp) {
                ...
                if(response.success == 0){
                    ...
                    $.notify("Leave Head Added",{
                                        type: 'success',
                                        allow_dismiss: false
                                    });
                    $(s).addClass("btn-success").removeClass('btn-default');
                    $('#loader').load(livesite+ 'LeavePolicy/index/');
                }
