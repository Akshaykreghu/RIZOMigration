...
<body style="font-family:times;">
    <div class="page-container">
        ...
        <?php if (!empty($attributeMarks) && !($is_drafted == 1 && $hideReporting == true) && ($is_rejected != 1)): ?>
            ...
            // $isRevOfficer   = ($loggedInUserId == $details['reviewing_officer']);
            $isRepOfficer   = $details['reporting_officer'];
            $isRevOfficer   = $details['reviewing_officer'];
            ?>
    ...
    <?php if (!empty($attributeMarks) && !($is_drafted == 1 && $hideReporting == true) && ($is_rejected != 1)): ?>
        <div class="page-container grading-container" style="font-family: times;margin-top:6mm;">
            ...
            <table class="final-assessment-table" cellspacing="0" cellpadding="6" style="width:100%; border-collapse: collapse;">
                <tr>
                    <th style="height: 10px; vertical-align: middle; width:<?= $colWidths[0] ?>;"></th>
                ...
                ?>
                    <tr>
                        ...
                        <td style="padding:5px;word-wrap: break-word; white-space: normal;min-height: <?= $rowHeight ?>;width:<?= $colWidths[1] ?>; vertical-align: top;"><?= htmlspecialchars($data['reportingData']) ?></td>
                        <?php if ($isRevOfficer): ?>
                            <td style="padding:5px;word-wrap: break-word; white-space: normal;min-height: <?= $rowHeight ?>;width:<?= $colWidths[2] ?>; vertical-align: top;">
                                <?php if (!($is_drafted == 1 && $hideReviewing)): ?>
                                    <?= htmlspecialchars($data['reviewingData']) ?>
                                <?php endif; ?>
                            </td>
                        <?php endif; ?>
                    </tr>
