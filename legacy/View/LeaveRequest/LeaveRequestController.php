...
//debug($arr_request_data);
    ...
    public function listempleaves() {
        ...
        $conditions = array(
            'OR' => array(
                array('ISAutherizedby' => $cur_emp_key, 'LEAVESTATUS IN("Applied","Cancelled")'),
    ...
    public function listempleavesverified() {
        ...
        $conditions = array(
            'OR' => array(
                ...
                array('ISAutherizedby' => $cur_emp_key, 'LEAVESTATUS IN("Authorized","Rejected","Approved")')
            )
        );
        ...
        if ($leaveentryId) {
            ...
            if ($this->checkIfLeaveRequestEditable($leaveentryId)) {
                ...
                if ($arr_leave_details['LeaveRequests']['ISAutherizedby'] == $cur_emp_key) {
                    ...
                    if ($arr_leave_details['LeaveRequests']['ISAutherized'] == 1 && !in_array($arr_leave_details['LeaveRequests']['LEAVESTATUS'], array('Cancellation Applied', 'Cancellation Authorized'))) {
                        $ISAutherized = 1;
                    }
                    $myrole = 1;
                }
                if ($arr_leave_details['LeaveRequests']['APPROVEDBY'] == $cur_emp_key) {
                    ...
                    if ($arr_leave_details['LeaveRequests']['ISAPPROVED'] == 1 && !in_array($arr_leave_details['LeaveRequests']['LEAVESTATUS'], array('Cancellation Applied', 'Cancellation Authorized'))) {
                        $ISAPPROVED = 1;
                    }
                    $myrole = 2;
                }
    ...
    public function saveLeaveEntry($leaveentryId = 0) {
        ...
        if ($leaveentryId) {
            ...
            if ($myLeaveAction == 'Cancellation Applied') {
                $data['message'] = '"Leave cancellation Applied"';
                ...
                $data['message'] = '"Leave cancellation Successful"';
                $data['FROMDATE'] = isset($arr_form_data['FROMDATE']) ? $arr_form_data['FROMDATE'] : '';
                $data['TODATE'] = isset($arr_form_data['TODATE']) ? $arr_form_data['TODATE'] : '';
            ...
            $arr_count_leave_exists = $this->LeaveRequests->query("select count(*) AS COUNT from emp_leave_transactions where leave_date between '$from_date' and '$to_date' and LEAVEENTRYID in (select LEAVEENTRYID  from leaveentries where EMP_fkey = $cur_emp_key)");
...
die($cur_emp_key);
        if ($cur_emp_key != 0 && $leaveentryId != 0) {
            ...
            if ($actionType == 'Authorize') {
                ...
                if ($leave_status == "Cancellation Applied" || $leave_status == "Cancellation Authorized") {
                    $data['LEAVESTATUS'] = "'Authorized'";
                    $outputParameter1['action'] = "Authorized";
                    $data['Autherized_date'] = "'" . $curdate . "'";
                    $data['ISAutherized'] = 1;
            ...
            }else if ($actionType == 'Cancellation Authorized') {
                $actionType = "Cancellation Authorize";
                $data['LEAVESTATUS'] = "'Cancellation Authorized'";
                $outputParameter1['action'] = "Cancellation Approval";
                $data['Autherized_date'] = "'" . $curdate . "'";
                $data['ISAutherized'] = 1;
    ...
    public function GetLeaveBalance($leavebalance = "", $end_date = "") {
        /* $this->autoRender = false;
          ...
          //$month_leavebal = '';
          //$month_leavebal = isset($lbalance1['0']['leavepolicy']['alloted_leave_forthe_month']) ? ',Monthly Balance : ' . $lbalance1['0']['leavepolicy']['alloted_leave_forthe_month'] : '0';
          //$yearly = isset($lbalance['0']['0']['LeaveBalance']) ? 'Yearly Balance:' . $lbalance['0']['0']['LeaveBalance'] : '';
          //$resp = $yearly . $month_leavebal;
          //return isset($resp) ? $resp : '';
          $yearly_balance = isset($lbalance['0']['0']['LeaveBalance']) ? $lbalance['0']['0']['LeaveBalance'] : 0;
          $monthly_balance = isset($lbalance1['0']['leavepolicy']['alloted_leave_forthe_month']) ? $lbalance1['0']['leavepolicy']['alloted_leave_forthe_month'] : 0;
        ...
        $get_finyear = $this->LeaveRequests->query("select fin_year from fin_year where '$end_date' between start_month and end_month");
        $finyear = isset($get_finyear['0']['fin_year']['fin_year']) ? $get_finyear['0']['fin_year']['fin_year'] : date('Y');
    ...
    public function getLeaveBalanceForAuthOrApproval($leaveentryId = "") {
        ...
        $get_finyear = $this->LeaveRequests->query("select fin_year from fin_year where '$from_date' between start_month and end_month");
        $finyear = isset($get_finyear['0']['fin_year']['fin_year']) ? $get_finyear['0']['fin_year']['fin_year'] : date('Y');
