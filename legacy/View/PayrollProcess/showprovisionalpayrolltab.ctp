<style>
    .tabset-processpayroll .pws_tabs_list{
            ...
            <table cellspacing="0" cellpadding="0">
                <tbody>
                    <tr>
                        ...
                        <td>
                            <a href="javascript:void(0)" onclick="showModalReversal();" class="l-btn l-btn-small l-btn-plain" group="" id="">
                                <span class="l-btn-left l-btn-icon-left">
        ...
        <table id="payrolltable" class="table table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th data-options="field:'reverse_remarks',width:'25%'">Remarks</th>
                </tr>
            </thead>
        </table>
        <?php }else{ ?>
        <table id="processedpayrolltable" class="table table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th data-options="field:'emp_name',width:'25%'" sortable="true">Employee Name</th>
                    <!--<th data-options="field:'calander_days',width:'8%'" sortable="true">Calender Days</th>-->
                    <th data-options="field:'working_days',width:'10%'" sortable="true">Working Days</th>
                    ...
                    <th data-options="field:'remarks',width:'25%'" sortable="true">Remarks</th>
                </tr>
            </thead>
        </table>
        <?php } ?>
...
<script>
    ...
    $('#payrolltable').datagrid({
        url: livesite + "PayrollProcess/listpayroll",
        pagination: true,
        singleSelect: false,
        queryParams: {
            branch: branch,
...
//                return 'color:#B22222;';
    ...
    function showModal() {
        ...
        if(row){
            // Show the modal
            showSmallModalForm(livesite + "PayrollProcess/addremarks/0");
        }else{
...
<script>
        ...
        for (var register in checkedRows) {
            if(checkedRows[register]['action'] == 'Approved'){
                canRemove = false;
                $msg= "You cannot remove an approved payroll entry!";
            }else{
             if(checkedRows[register]['final_status'] == 'audited'){
                canRemove = false;
                $msg= "You cannot remove a finance vetting processed entry!";
                break;
              }
            }
        }
         if(canRemove){
            // Show the modal
            showSmallModalForm(livesite + "PayrollProcess/addreversalremarks/0");
    ...
    $('#processedpayrolltable').datagrid({
        url: livesite + "PayrollProcess/listprovisionalpayroll",
        pagination: true,
        singleSelect: false,
        queryParams: {
            branch: branch,
