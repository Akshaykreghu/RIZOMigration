...
<section class="content-header">
    <h1 class="text-primary-18">Provisional Payroll </h1>
...
<script>
    jQuery(document).ready(function () {
        ...
        $('.tabset-processpayroll #tab1').load(livesite + "PayrollProcess/showprovisionalpayrolltab/0");
    });
	...
	jQuery('#div-showprocesspayroll .pws_tabs_controll a').on('click', function(){
		...
		if(tabIndex == "tab1"){
			...
			$('.tabset-processpayroll #tab2').load(livesite + "PayrollProcess/showprovisionalpayrolltab/1");
		}
	});
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (success) {
                    ...
                    $('.tabset-processpayroll #tab1').load(livesite + "PayrollProcess/showprovisionalpayrolltab/0", {month: month, branch: branch});
                    $('.tabset-processpayroll #tab2').load(livesite + "PayrollProcess/showprovisionalpayrolltab/1", {month: month, branch: branch});
    ...
    function processPayroll(remarks) {
        ...
        $.ajax({
            url: livesite + "PayrollProcess/processpayroll",
            type: 'post',
            data: {
                branch: branch,
                month: month,
            ...
            success: function (response) {
                ...
                if (success) {
                    ...
                    if (remark !== ''){
                    $.notify("Provisional payroll processed.", {
                        type: 'success',
                        allow_dismiss: false,
                    });
    ...
    function removePayrollEntry(remarks) {
        var canRemove = true;
        console.log('Remarks', remarks);
        var branch = $('#payrollfilter #filterby_branch').val();
        var month = $('#payrollfilter #filterby_month').val();
        //Edited by Akshay on 25-7-2023
        var remarks =remarks;
        var arr_emp_pkeys = [];
        var arr_payroll_pkeys = [];
        var checkedRows = $('#processedpayrolltable').datagrid('getChecked');
        for (var register in checkedRows) {
            if(checkedRows[register]['action'] == 'Approved'){
             ...
             if(checkedRows[register]['final_status'] == 'audited'){
                canRemove = false;
                break;
              }
            }
        ...
        if(canRemove){
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (success) {
                        $('#remove1').modal('hide');
                        $.notify("Provisional payroll removed.", {
                            type: 'success',
                            allow_dismiss: false
                        });
                        $('#processedpayrolltable').datagrid('load', {
                        ...
                        //alert('Attendance process failed!');
                        $.notify("Provisional payroll removal failed.", {
                            type: 'danger',
                            allow_dismiss: false
                        });
                    }
            ...
            $.notify("You cannot remove an finance vetting processed entry!", {
                type: 'danger',
                allow_dismiss: false
            });
        }
