...
<section class="content-header">
    <h1 class="text-primary-18">Finance Vetting</h1>
...
<script>
    jQuery(document).ready(function () {
        ...
        $('.tabset-processpayroll #tab1').load(livesite + "PayrollProcess/showpreauditpayrolltab/0");
    });
	...
	jQuery('#div-showprocesspayroll .pws_tabs_controll a').on('click', function(){
		...
		if(tabIndex == "tab1"){
			...
			$('.tabset-processpayroll #tab2').load(livesite + "PayrollProcess/showpreauditpayrolltab/1");
		}
	});
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (success) {
                    ...
                    $('.tabset-processpayroll #tab1').load(livesite + "PayrollProcess/showpreauditpayrolltab/0", {month: month, branch: branch});
                    $('.tabset-processpayroll #tab2').load(livesite + "PayrollProcess/showpreauditpayrolltab/1", {month: month, branch: branch});
    ...
    function auditPayroll(remarks,dataToPass) {
        ...
        $.ajax({
            url: livesite + "PayrollProcess/preauditpayroll",
            type: 'post',
            data: {
                branch: branch,
                month: month,
            ...
            success: function (response) {
                ...
                if (success) {
                    ...
                    if (remark !== ''){
                    $.notify("Finance vetting processed.", {
                        type: 'success',
                        allow_dismiss: false,
                    });
        ...
        if(canRemove){
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var success = $.parseJSON(response).success;
                    if (success) {
                        $.notify("Finance vetting removed.", {
                            type: 'success',
                            allow_dismiss: false
                        });
                        $('#processedpayrolltable').datagrid('load', {
                        ...
                        //alert('Attendance process failed!');
                        $.notify("Finance vetting removal failed!", {
                            type: 'danger',
                            allow_dismiss: false
                        });
                    }
            ...
            $.notify("You cannot remove a finalized payroll!", {
                type: 'danger',
                allow_dismiss: false
            });
        }
