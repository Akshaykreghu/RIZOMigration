<?php if (isset($arr_reportcriterias)) { ?>
    ...
    <form class="form-horizontal" method="post" action="" id="form-showreport">
        ...
        <div class="form-group" id="div-criteria1">
            ...
            <?php if ($type == "ProfTax" || $type == "ProfSalary") { ?>
                <!-- end -->
            ...
            <?php } else if (($type == "TDS" || $type == "TAX") && $type != "EPF_NEW" && $type != "ESI_NEW" && $type != "12BB") { ?>
                <div class="col-md-4">
            ...
            <?php } elseif ($type != "TAX" || $type == "EPF_NEW" || $type == "ESI_NEW" || $type == "ESI" || $type == "PF" || $type == "12BB" || $type == "PF_UPLOAD_SYNTHIET") { ?> <!-- Edited by Ashin on 12-8-2024 -->
            ...
            <?php if ($type == "Labour") { ?>
                <div class="col-md-2">
                    <select id="report_component" name="report_component" style="width:130px;" class="form-control">
                        <option value="">--Choose Type--</option>
                        <option value="esi"> ESI EPF</option>
                        <option value="tax"> TDS </option>
                        <option value="protax"> Professional Tax </option>
                    </select>
        ...
        <div class="form-group">
            <div class="col-md-12" align="right">
                <!-- edited by athira on 18-04-2025 -->
                <?php if ($type != 'ProfSalary') { ?>
                    <button type="button" onclick="viewReport();" id="btn-submit" class="btn btn-primary">
                        <li class="fa fa-eye"></li>
                    </button>
                <?php } ?>
                <?php if ($type != "TAX" && $type != "PF_UPLOAD_SYNTHIET" && $type != 'ProfSalary') { ?>
                ...
                <?php } ?>
                <?php if ($type == "EPF_SYNTHIET") { ?>
                    <button type="button" id="btn-submit3" class="btn btn-info">
                        <li class="fa fa-print"></li>
                    </button>
                <?php } ?>
                <?php if ($type == "ESI_SYNTHIET") { ?>
                    <button type="button" id="btn-submit4" class="btn btn-info">
                        <li class="fa fa-print"></li>
                    </button>
                <?php } ?>
                <!-- Edited by Ashin on 12-8-2024 -->
                <?php if ($type == "PF_UPLOAD_SYNTHIET") { ?>
                    <button type="button" id="btn-submit5" class="btn btn-info">
                        <li class="fa fa-print"></li>
                    </button>
                <?php } ?>
                <!-- End -->
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-submit4').click(function() {
                var content = $('#printableEsiContent').html();
                printESIContent(content);
            });
            $('#btn-submit5').click(function() {
                var content = $('#printableContent').html();
                printContentnew(content);
            });
        });
        ...
        function printContent(content) {
            ...
            printWindow.document.write('th { background-color: #f2f2f2; white-space: nowrap; }');
            printWindow.document.write('tr.no-border-print, th.no-border-print, fieldset { border: none; margin: 0; padding: 0; }');
            printWindow.document.write('legend { font-size: 1.2em; margin-bottom: 10px; }');
        ...
        function printContentnew(content) {
            var printWindow = window.open('', '_blank', 'height=500,width=1200');
            printWindow.document.write('<html><head><title>Print Content</title>');
            printWindow.document.write('<style>body { font-size: 12pt; } table { width: 100%;} th, td { border: 1px solid black; padding: 8px; text-align: left; } th { background-color: #f2f2f2; } tr.no-border-print,th.no-border-print,fieldset { border: none; margin: 0; padding: 0; } legend { font-size: 1.2em; margin-bottom: 10px; }</style>');
            printWindow.document.write('</head><body>');
            printWindow.document.write(content);
            printWindow.document.write('</body></html>');
            printWindow.document.close();
        ...
        function printESIContent(content) {
            var printWindow = window.open('', '_blank', 'height=600,width=900');
            ...
            printWindow.document.write('th, td { padding: 0; text-align: left; }');
            printWindow.document.write('th { background-color: #f2f2f2; text-align: center; }');
            printWindow.document.write('tr, th, td { border: none; }');
            ...
            printWindow.document.write('@page { size: auto; margin: 0mm; }'); // Hide browser default headers/footers
            printWindow.document.write('</style>');
            printWindow.document.write('</head><body>');
            printWindow.document.write(content);
            printWindow.document.write('</body></html>');
        ...
        function loadCriteriaItems(index) {
            ...
            $('#div-items-criteria' + index).load(livesite + 'StatutoryReport/loadcriteriaitems/' + index + '/' + criteria);
        }
        ...
        function downloadReport(mode) {
            ...
            if (criteria) {
                $('#loaders').show();
                $('#form-showreport').attr('action', livesite + 'StatutoryReport/generatereport/' + type + '/' + mode);
        ...
        <?php if ($type == 'employee') { ?>
            ...
            allempfields.init();
            allempfields.clearAndLoad("<?php echo $this->webroot; ?>StatutoryReport/listemployeefields", "json");
        ...
        //    if(criteria){
        //          $('#loaders').show();
        //        var url = livesite+'StatutoryReport/generatereport/'+type;
        ...
        function viewReport() {
            ...
            if (selectany) {
                ...
                if (criteria) {
                    ...
                    $.post(url, $('#form-showreport').serialize(), function(data) {
                        $('#reportCon').html(data);
                        $('#loaders').hide(); // hide the loader here
                        if (type === "EPF_SYNTHIET") {
                            $('#btn-submit3').show();
                        }
                        if (type === "ESI_SYNTHIET") {
                            $('#btn-submit4').show();
                        }
                        if (type === "PF_UPLOAD_SYNTHIET") {
                            $('#btn-submit5').show();
                        }
                    });
                } else {
        ...
        function addOneReportCriteria(index) {
            ...
            $('#div-criteria' + newIndex).load(livesite + 'StatutoryReport/addreportcriteria/' + type + '/' + newIndex + '/' + currentcriteriaschosen);
