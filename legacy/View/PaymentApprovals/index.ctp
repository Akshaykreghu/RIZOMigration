...
<div class="payroll-view">
  <section class="content-header heading">
      <h1 class="text-primary-18"> Payment Approvals</h1>
  </section>
...
$(document).ready(function () {
  ...
  const pageKey = "payment"; // unique key for this page
  const storageKey = `last_clicked_feature_${pageKey}`;

  // ===== Fetch Payment Features =====
  $.getJSON("PaymentApprovals/getPaymentFeatures", function (res) {
    if (res.error) {
    ...
    // ===== Restore last clicked or select first active =====
  ...
  $(document).on("click", ".payroll-main-grid", function (e) {
    ...
    let enabled = parseInt($(this).data("enabled")); // ensure number
    ...
    if (enabled === 1) {
      ...
      if (article) {
        ...
        $.each(links, function(i, link) {
          link = link.trim();
          if (link) {
            let text = decodeURIComponent(link.split("/").pop().replace(/-/g, " "));
