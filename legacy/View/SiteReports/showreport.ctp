<?php if (isset($arr_reportcriterias)) { ?>
    ...
    <form class="form-horizontal" method="post" action="" id="form-showreport">
        ...
        <div class="form-group" id="div-criteria1">
           ...
           <div class="col-md-5">
            ...
            <div class="col-md-2">
                <select id="select-criteria1" name="select-criteria1" style="width: 180px;" class="form-control" onchange="loadCriteriaItems(1);">
            ...
            <div class="<?php echo ($type == 'ClientReport(Actual)') ? 'col-md-3' : 'col-md-3'; ?>" id="div-items-criteria2">
        ...
        <div id="printPanel" class="form-group">
            <div class="col-md-12" align="right">
                ...
                <?php if ($type == "EmployeePayHours" || $type == "AssignmentAttendance") { ?>
                    <!-- <button type="button" onclick="downloadReport('pdf');" id="btn-submit1" class="btn btn-danger">
                        <li class="fa fa-file-pdf-o"></li>
                    </button> -->
                <?php } else  ?>
                <?php if ($type == "ClientReportStandard" || $type == "ClientReport(Actual)") { ?>
                    <!-- <button type="button" onclick="downloadReport('pdf');" id="btn-submit1" class="btn btn-danger tooltip2">
                        <li class="fa fa-file-pdf-o"></li> <span class="tooltiptext">Maximum 10 records only show in pdf.</span>
                    </button> -->
                <?php } ?>
    ...
    <script>
        function loadCriteriaItems(index) {
            ...
            $('#div-items-criteria' + index).load(livesite + 'SiteReports/loadcriteriaitems/' + index + '/' + criteria);
        }
        ...
        function downloadReport(mode) {
            ...
            if (criteria) {
                $('#form-showreport').attr('action', livesite + 'SiteReports/generatereport/' + type + '/' + mode);
                $('#form-showreport').submit();
                if (type === "ClientReport(Actual)" && criteria === "Units" && mode === 'pdf') {
                    $('#hidden-criteria1').val($('#select-criteria1').val());
                    loadCriteriaItems(1);
                } else {
                    // loadCriteriaItems(1);
                }
        ...
        <?php if ($type == 'employee') { ?>
            ...
            allempfields.init();
            allempfields.clearAndLoad("<?php echo $this->webroot; ?>SiteReports/listemployeefields", "json");
        ...
        function viewReport() {
            ...
            var url = livesite + 'SiteReports/generatereport/' + type;
...
<style>
    ...
    .tooltip2 .tooltiptext::after {
        ...
        border-width: 5px;
        border-style: solid;
        border-color: transparent transparent #32659f transparent;
    }
