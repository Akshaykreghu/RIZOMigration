...
// $('#date').hide();
  ...
  var v = $('#filterby_reporttype').val();
       if(v == 'ProjectExpense' || v =='BeneficiaryExpense' )
{
      $('#date').show();
}
else
...
<form class="form-horizontal" method="post" action="" id="form-showreport">
    ...
    <div class="form-group" id="div-criteria1">
      ...
      <div class="col-md-5" id="items-criteria1" >
...
function downloadReport(mode){
	...
	var criteria = $('#select-criteria1').val();
        ...
        var proany = false;
            ...
            if (selectany) {
            var item = $('#reporstto').val();
         if(item == ''){
            alert("Please choose end date");
            return false;
         }
         if(cri == 'Beneficiary' || cri == 'ExpenseType'){
         if (proany) {
	...
	if(criteria){
        $('#form-showreport').attr('action',livesite+'ProjectExpenseReport/generatereport/'+type+'/'+mode);
 ...
 //load item names from selected store
    function  loadItems() {
        ...
        var type = $('#hidden-report-type').val();
	var criteria = $('#select-criteria1').val();
        var url = livesite+'ProjectExpenseReport/itemlist/'+type;
        //var criteria = $('#select-criteria1').val();
        //alert(criteria);
          $("#items-criteria").select2(
                {
                    ...
                    allowClear: true,
                    ajax: {
                        url: livesite + "ProjectExpenseReport/itemlist/" ,
                        dataType: 'json',
                        delay: 250,
...
function viewReport(){ 
	...
	if(criteria){
        ...
        var url = livesite+'ProjectExpenseReport/generatereport/'+type;
...
function loadCriteriaItems(index){
    ...
    var type = $('#hidden-report-type').val();
    $('#div-items-criteria'+index).load(livesite+'ProjectExpenseReport/loadcriteriaitems/'+index+'/'+criteria);
    if(type == "ProjectExpense"){
     $('#items-criteria').show();
    }
    if(criteria == ''){
     $('#items-criteria1').hide(); 
    }
...
function addOneReportCriteria(index){
    ...
    $('#div-criteria'+newIndex).load(livesite+'ProjectExpenseReport/addreportcriteria/'+type+'/'+newIndex+'/'+currentcriteriaschosen);
