...
<script>
    $(document).ready(function() {
        ...
        $("#avatarfile").fileinput({
            uploadUrl: livesite + "DocumentManagers/saveImage/<?= $id; ?>",
            showCaption: false,
            showClose: true,
    ...
    function loaditem() {
        showLargeModalForm(livesite + 'DocumentManagers/renderImageTempaltePrew/<?= $id; ?>/<?= $companycode; ?>')
    }
    ...
    function saveData() {
        ...
        $.ajax({
            type: "POST",
            url: livesite + "DocumentManagers/save_template",
            data: {
                'data': data,
