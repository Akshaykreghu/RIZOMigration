...
<script>
 ...
 $('#date').hide();
  ...
  var v = $('#filterby_reporttype').val();
       if(v == 'SummaryPayroll' || v =='Salaryslip')
{
      $('#date').show();
}
else
...
<form class="form-horizontal" method="post" action="" id="form-showreport">
    ...
    <div class="form-group" id="div-criteria1">
        ...
        <div class="col-md-1"><b>Criteria<span style="padding-left: 98px;">:</span></b> </div>
        <div class="col-md-4" style="padding-left: 77px;">
            <select id="select-criteria1" style="width: 240px;" name="select-criteria1" class="form-control" onchange="loadCriteriaItems(1);" >
...
function downloadReport(mode){
	...
	if(criteria){
        $('#form-showreport').attr('action',livesite+'AssetsReports/generatereport/'+type+'/'+mode);
...
function viewReport(){ 
     ...
     ///   $("#reportCon").html('<li class="fa fa-spinner fa-spin" style="margin-left: 8em;margin-top: 2em;font-size: 73px;"></li>');
        var url = livesite+'AssetsReports/generatereport/'+type;
...
function loadCriteriaItems(index){
    ...
    $('#div-items-criteria'+index).load(livesite+'AssetsReports/loadcriteriaitems/'+index+'/'+criteria);
}
function addOneReportCriteria(index){
    ...
    $('#div-criteria'+newIndex).load(livesite+'AssetsReports/addreportcriteria/'+type+'/'+newIndex+'/'+currentcriteriaschosen);
